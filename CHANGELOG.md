...
* **Security, performance and memory audit (57 findings).** Four critical:
  `/_replica` and the shard-direct branches trusted the mere presence of
  `X-Shard-Direct` and a wrong cluster secret fell through to user auth, and
  offline-sync push wrote by name without the write tiers — either let a
  principal with Write reach `_triggers`/`_jobs`/`_user_roles` and run Lua as
  `_system`; resetting a pooled Lua state restored which table `crypto`/`json`
  named but not their fields, so `crypto.verify_password = …` leaked into the
  next tenant; `ROW_POLICY(coll, null)` lifted a policy with only Read. Also:
  the query result cache is keyed per principal, row policies apply on every
  read path, sync pull no longer serves `_admins` hashes, Basic and driver auth
  share the login limiter with Argon2 capped per core, single queries can no
  longer abort or pin the server (`VECTOR_SEARCH` k, `LEVENSHTEIN`, `DATE_ADD`,
  `WINDOW "0s"`), inserting an existing `_key` returns 409, transaction commit
  is validated then atomic, replication batches are sized by bytes, and the
  reapers that were never scheduled now run. See the docs-site changelog for
  the full list and `CLAUDE.md` for the new `SOLIDB_*` bounds.
...
* **Rust client decodes query results once.** The driver response was an
  internally tagged enum, so serde buffered the whole message before choosing
  the variant, and `query` then re-deserialized every row with
  `serde_json::from_value`. A flat wire struct is decoded straight from the
  bytes and the row array is returned as is; the wire encoding is unchanged.
