...
impl HttpClient {
    ...
    pub async fn login(
        ...
        let data: Value = response.json().await.map_err(|e| {
            DriverError::ProtocolError(format!("Failed to parse login response: {}", e))
        })?;
        ...
        T: serde::de::DeserializeOwned,
    {
        ...
        if !status.is_success() {
            ...
            return Err(DriverError::ServerError(format!(
                "HTTP {} {}: {}",
        ...
        if text.is_empty() {
            return Err(DriverError::ServerError(format!(
                "Empty response for HTTP {} {}",
                method, path
            )));
        }

        serde_json::from_str(&text).map_err(|e| {
            DriverError::ProtocolError(format!("Failed to parse response: {} - Text: {}", e, text))
        })
    }
    ...
    pub async fn list_collections(
        ...
        let db = database
            ...
            .ok_or_else(|| DriverError::ProtocolError("No database specified".to_string()))?;
        let response: Value = self
            .request("GET", &format!("/_api/database/{}/collection", db), None)
            .await?;
        Ok(response
            .get("collections")
    ...
    pub async fn begin_transaction(
        ...
        let response: Value = self
            .request("POST", "/_api/transaction/begin", Some(&payload))
            .await?;
        response
            .get("tx_id")
            .and_then(|t| t.as_str())
