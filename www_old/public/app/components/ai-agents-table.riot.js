...
export default {
  ...
  exports: {
    ...
    async loadAgents() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.update({ agents: data.agents, loading: false })
            }
        } catch (e) {
            console.error(e)
            if (this.state.agents.length === 0) {
    ...
    async unregister(agent) {
        ...
        try {
            ...
            if (response.ok) this.loadAgents()
            else {
  ...
  template: (
    ...
    '<div class="bg-gray-800 shadow-xl rounded-lg overflow-hidden border border-gray-700"><div class="p-4 border-b border-gray-700 flex justify-between items-center bg-gray-800/50 backdrop-blur-sm"><div class="flex space-x-4"><div class="relative"><select expr304="expr304" class="appearance-none bg-gray-900 border border-gray-600 text-gray-300 py-2 pl-4 pr-8 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"><option value>All Statuses</option><option value="idle">Idle</option><option value="busy">Busy</option><option value="offline">Offline</option><option value="error">Error</option></select><div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-400"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg></div></div></div><div class="flex space-x-2"><span expr305="expr305" class="flex items-center text-xs text-green-400"></span><button expr306="expr306" class="text-gray-400 hover:text-white transition-colors p-2 rounded-full hover:bg-gray-700"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg></button></div></div><div expr307="expr307" class="flex justify-center items-center py-12"></div><div expr308="expr308" class="text-center py-12"></div><div expr310="expr310" class="text-center py-12"></div><div expr311="expr311" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4"></div></div>',
    [
      {
      ...
      },
      {
        ...
        template: template(
          '<div class="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-500"></div><span class="ml-3 text-gray-400">Loading agents...</span>',
          []
        )
      },
      {
        ...
        template: template(
          '<p expr309="expr309" class="text-red-400"> </p>',
          [
            {
      ...
      },
      {
        ...
        template: template(
          '<div expr312="expr312" class="bg-gray-750 rounded-lg p-5 border border-gray-700 hover:border-gray-600 transition-colors"></div>',
          [
            {
              ...
              template: template(
                '<div class="flex justify-between items-start mb-4"><div class="flex items-center"><div class="h-10 w-10 rounded-lg bg-indigo-900/50 flex items-center justify-center text-indigo-400"><svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.384-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/></svg></div><div class="ml-3"><h3 expr313="expr313" class="text-sm font-medium text-white"> </h3><p expr314="expr314" class="text-xs text-gray-400 capitalize"> </p></div></div><span expr315="expr315"> </span></div><div class="space-y-3"><div class="grid grid-cols-2 gap-2 text-xs"><div class="bg-gray-800 p-2 rounded"><div class="text-gray-500">Completed</div><div expr316="expr316" class="text-gray-200 font-semibold"> </div></div><div class="bg-gray-800 p-2 rounded"><div class="text-gray-500">Failed</div><div expr317="expr317" class="text-gray-200 font-semibold"> </div></div></div><div expr318="expr318" class="text-xs"></div><div class="flex justify-between items-center text-xs text-gray-500 pt-2 border-t border-gray-700"><span expr320="expr320"> </span><button expr321="expr321" class="text-red-400\n                            hover:text-red-300">Unregister</button></div></div>',
                [
                  {
                    ...
                    expressions: [
                      ...
                      },
                      {
                        ...
                        evaluate: _scope => [
                          ...
                          'agent.status === \'busy\' ? \'bg-blue-900/30 text-blue-400 animate-pulse\' :\\n               agent.status === \'idle\' ? \'bg-green-900/30 text-green-400\' :\\n               \'bg-gray-700 text-gray-400\''
                        ].join(
                          ''
                        )
                      }
                  ...
                  },
                  {
                    ...
                    template: template(
                      '<div class="text-gray-500 mb-1">Processing</div><div expr319="expr319" class="bg-gray-800 p-2 rounded text-blue-300 truncate font-mono"> </div>',
                      [
                        {
                    ...
                    expressions: [
                      {
                        ...
                        evaluate: _scope => [
                          'Last seen: ',
                          new Date(_scope.agent.last_heartbeat).toLocaleTimeString()
                        ].join(
                          ''
                        )
                      }
