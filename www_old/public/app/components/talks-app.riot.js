...
import Thread from '/app/components/talks-thread.riot.js'
import { getApiUrl } from '/api-config.js'

export default {
  css: `talks-app,[is="talks-app"]{ display: block; height: 100vh; height: 100dvh; }talks-app ::-webkit-scrollbar,[is="talks-app"] ::-webkit-scrollbar{ width: 8px; }talks-app ::-webkit-scrollbar-track,[is="talks-app"] ::-webkit-scrollbar-track{ background: transparent; }talks-app ::-webkit-scrollbar-thumb,[is="talks-app"] ::-webkit-scrollbar-thumb{ background: #36393E; border-radius: 4px; }talks-app ::-webkit-scrollbar-thumb:hover,[is="talks-app"] ::-webkit-scrollbar-thumb:hover{ background: #4B4F54; }talks-app .hover\\:bg-\\[\\#350D36\\]:hover,[is="talks-app"] .hover\\:bg-\\[\\#350D36\\]:hover{ transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); }talks-app .ace_editor,[is="talks-app"] .ace_editor{ background-color: transparent !important; }talks-app .ace_gutter,[is="talks-app"] .ace_gutter{ background-color: rgba(26, 29, 33, 0.5) !important; color: #4B4F54 !important; } @keyframes fade-in { from { opacity: 0; } to { opacity: 1; } }talks-app .animate-fade-in,[is="talks-app"] .animate-fade-in{ animation: fade-in 0.2s ease-out; } @keyframes slide-in-right { from { transform: translateX(100%); opacity: 0; } to { transform: translateX(0); opacity: 1; } }talks-app .animate-slide-in-right,[is="talks-app"] .animate-slide-in-right{ animation: slide-in-right 0.3s ease-out; }talks-app .line-clamp-2,[is="talks-app"] .line-clamp-2{ display: -webkit-box; -webkit-line-clamp: 2; line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; } @media (prefers-reduced-motion: reduce) {talks-app .animate-fade-in,[is="talks-app"] .animate-fade-in,talks-app .animate-slide-in-right,[is="talks-app"] .animate-slide-in-right,talks-app .animate-fade-in-up,[is="talks-app"] .animate-fade-in-up{ animation: none; }talks-app .hover\\:scale-105:hover,[is="talks-app"] .hover\\:scale-105:hover{ transform: none; }talks-app .active\\:scale-95:active,[is="talks-app"] .active\\:scale-95:active{ transform: none; } } @media (max-width: 639px) {talks-app .animate-fade-in,[is="talks-app"] .animate-fade-in,talks-app .animate-slide-in-right,[is="talks-app"] .animate-slide-in-right,talks-app .animate-fade-in-up,[is="talks-app"] .animate-fade-in-up{ animation-duration: 0.15s; } }`,
  ...
  exports: {
    ...
    calculateUnreadChannels() {
        ...
        const allChannels = [...(this.state.channels || []), ...(this.props.dmChannels || [])];
    ...
    getFavorites() {
        ...
        favorites.forEach(key => {
            const channel = this.state.channels.find(c => c._key === key);
            if (channel) {
    ...
    async openThread(message, e) {
        ...
        try {
            const response = await fetch(`/talks/get_thread_messages?parent_id=${message._id}`);
            const data = await response.json();
            if (data.success) {
                this.update({
    ...
    handleThreadReplySent(newMessage) {
        ...
        const messages = this.state.messages.map(msg => {
            if (msg._id === this.state.threadParentMessage._id) {
                ...
                return { ...msg, thread_count: threadCount, thread_participants: participants };
            }
            return msg;
        });
    ...
    // Get local stream for passing to child components
    getLocalStream() {
        return this.localStream || null;
    },

    // Get LiveQuery token - uses cached token from server initially, re-fetches when expired
    async getLivequeryToken() {
        // Use cached token if still valid (refresh 5 seconds before expiry)
        if (this._cachedToken && this._tokenExpiry && Date.now() < this._tokenExpiry - 5000) {
            return this._cachedToken;
        }

        // Use token from props on first load
        if (!this._cachedToken && this.props.livequeryToken) {
            this._cachedToken = this.props.livequeryToken;
            this._tokenExpiry = Date.now() + 30000; // Token expires in 30 seconds
            return this._cachedToken;
        }
        ...
        try {
            ...
            this._tokenExpiry = Date.now() + (expires_in || 30) * 1000;
            return token;
        } catch (e) {
            console.error('Failed to fetch livequery token:', e);
            return null;
        }
    },
    ...
    initResponsive() {
        this.handleResize = () => {
            ...
            const screenSize = width < 640 ? 'mobile' : width < 1024 ? 'tablet' : 'desktop';

            this.update({
                isMobile,
                screenSize,
                showMobileSidebar: !isMobile ? false : this.state.showMobileSidebar
            });
        };
        ...
        this.handleTouchMove = (e) => {
            if (!this.state.isMobile || !this.touchStartX) return;
            ...
            if (Math.abs(diffX) > Math.abs(diffY) && Math.abs(diffX) > 50) {
                if (diffX > 0 && !this.state.showMobileSidebar) {
                    // Swipe right - open sidebar
                    this.update({ showMobileSidebar: true });
                } else if (diffX < 0 && this.state.showMobileSidebar) {
                    // Swipe left - close sidebar
                    this.update({ showMobileSidebar: false });
                }
    ...
    toggleMobileSidebar() {
        this.update({ showMobileSidebar: !this.state.showMobileSidebar });
    },
    ...
    onMounted() {
        ...
        // Sound permission popup always on load
        this.update({ showSoundPopup: true });

        // Request notification permission for DMs and mentions
        if ('Notification' in window && Notification.permission === 'default') {
            // Delay request to avoid overwhelming user
            setTimeout(() => {
        ...
        if (this.state.isMobile) {
            ...
            window.addEventListener('resize', this.handleVirtualKeyboard);
            window.addEventListener('orientationchange', this.handleVirtualKeyboard);
        }

        // Connect to users Live Query for real-time presence UI updates
        this.connectUsersLiveQuery();

        // Connect to channels Live Query
        this.connectChannelsListLiveQuery();
    ...
    onUnmounted() {
        ...
        if (this.handleVirtualKeyboard) {
            window.removeEventListener('resize', this.handleVirtualKeyboard);
            window.removeEventListener('orientationchange', this.handleVirtualKeyboard);
        }
    },
    ...
    initAudioContext() {
        ...
        if (this.audioCtx.state === 'suspended') {
            this.update({ audioSuspended: true });
        }
    ...
    async enableAudio() {
        ...
        this.update({ audioSuspended: false });
    },

    initRouter() {
        this.router = new Navigo('/talks', { hash: false });
    ...
    async switchChannel(channelKey, highlightMsgId) {
        if (this.state.currentChannelData && this.state.currentChannelData._key === channelKey) {
            if (highlightMsgId) {
                this.update({ highlightMessageId: highlightMsgId });
            }
            return;
        }
        ...
        this.update({ highlightMessageId: highlightMsgId || null });
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (unread[data.channelId]) {
                    ...
                    this.update({ unreadChannels: unread });
                }
            }
        } catch (err) {
    ...
    onBeforeUnmount() {
        ...
        if (this.channelsListWs) {
            this.channelsListWs.onclose = null;
            this.channelsListWs.close();
            this.channelsListWs = null;
        }
        this.hangup();
        this.hangup();
    },
    ...
    async connectChannelLiveQuery(channelId) {
        ...
        try {
            const token = await this.getLivequeryToken();
            if (!token) return;
    ...
    async connectChannelsListLiveQuery() {
        try {
            const token = await this.getLivequeryToken();
            if (!token) return;
            ...
            const wsProtocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
            const wsUrl = `${wsProtocol}//${this.getDbHost()}/_api/ws/changefeed?token=${token}`;
            this.channelsListWs = new WebSocket(wsUrl);

            this.channelsListWs.onopen = () => {
                console.log('Channels List Live Query connected');
                const my_user_key = this.props.currentUser._key;
                const query = `
                    LET my_user_key = "${my_user_key}"
                    FOR c IN channels
                        FILTER c.type == "system" OR c.type == "standard" OR POSITION(TO_ARRAY(c.members), my_user_key) >= 0
            ...
            this.channelsListWs.onmessage = (event) => {
                try {
                    const data = JSON.parse(event.data);
                    if (data.type === 'query_result' && data.result) {
                        const newChannels = data.result;
                        const updates = { channels: newChannels };

                        // Update current channel data if it exists in the new list
                        // This ensures huddle status (active_call_participants) is updated in real-time
                        if (this.state.channelId) {
                            const updatedCurrentChannel = newChannels.find(c => c._id === this.state.channelId);
                            if (updatedCurrentChannel) {
                                updates.currentChannelData = updatedCurrentChannel;
                                const newParticipants = updatedCurrentChannel.active_call_participants || [];
                                this.activeCallParticipants = newParticipants;

                                // Reconcile: If we are in a call, remove peers who are no longer in the participants list
                                if (this.state.activeCall) {
                                    Object.keys(this.peerConnections).forEach(peerKey => {
                                        if (!newParticipants.includes(peerKey)) {
                                            console.log(`[Call] User ${peerKey} removed from server list. Closing connection.`);
                                            this.closePeerConnection(peerKey);
                                        }
                                    });
                                }
                            } else if (this.state.currentChannelData && this.state.currentChannelData.type === 'private') {
                                // Current channel is private and no longer in our authorized list
                                console.log('[Channels] Removed from private channel. Switching to general.');
                                this.router.navigate('?channel=general');
                            }
                        }
                    ...
                    console.log('Channels List Live Query parse error:', err);
                }
            };

            this.channelsListWs.onclose = () => {
                console.log('Channels List Live Query disconnected');
                setTimeout(() => this.connectChannelsListLiveQuery(), 5000);
            };
        } catch (err) {
            console.error('Channels List Live Query error:', err);
            setTimeout(() => this.connectChannelsListLiveQuery(), 5000);
        }
    },
    ...
    // Connect to WebSocket using configured host
    async connectLiveQuery() {
        try {
            const token = await this.getLivequeryToken();
            if (!token) throw new Error('LiveQuery token fetch failed');
            ...
            this.ws.onopen = () => {
                ...
                const query = `
                    ...
                    LET my_channels = (
                        FOR c IN channels
                            FILTER c.type == "standard" OR POSITION(TO_ARRAY(c.members), my_user_key) >= 0
                            RETURN c._id
                    )
                    FOR m IN messages
                ...
                try {
                    ...
                    if (data.type === 'query_result' && data.result) {
                        ...
                        newMessages.forEach(m => {
                            if (!currentKeys.has(m._key)) {
                                ...
                                if (this.state.initialSyncDone && m.sender !== this.getUsername(this.props.currentUser)) {
                                    ...
                                    // Check if this is a DM from a channel we're not viewing
                                    const messageChannel = this.state.channels?.find(c => c._id === m.channel_id);
                                    const isDM = messageChannel?.type === 'dm';
                                    const isViewingThisChannel = this.state.currentChannel === messageChannel?._key;

                                    if (isMention || isQuote) {
                                        this.playSound('notification');
                                        this.sendBrowserNotification(m, 'mention');
                                    } else if (isDM && !isViewingThisChannel) {
                                        // DM from another channel we're not viewing
                                        this.playSound('notification');
                                        this.sendBrowserNotification(m, 'dm');
                        ...
                        if (hasNewItems || hasUpdates || hasDeletions) {
                                ...
                                newMessages.forEach(m => {
                                    if (!currentKeys.has(m._key)) {
                                        ...
                                        if (mentionsChannelId && this.state.currentChannel !== 'mentions') {
                                            ...
                                            const isMention = m.text && (
                                                m.text.includes('@' + username) ||
                                                m.text.includes('@' + email) ||
                                                m.text.includes('@' + emailPrefix)
                                            );
                            ...
                            const updateData = {
                                allMessages: updated,
                                messages: filtered, // This needs to be a new array reference for Riot to detect change
    ...
    handleQuoteMessage(message, e) {
        if (e) e.stopPropagation();
        this.update({ quotedMessage: message });
        const textarea = this.root.querySelector('textarea');
        if (textarea) textarea.focus();
    },

    cancelQuote() {
        this.update({ quotedMessage: null });
    },
    ...
    async updateMessage(messageKey, text) {
        try {
            const response = await fetch('/talks/update_message', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ message_key: messageKey, text: text })
            });
            if (response.ok) {
                // Immediate local update
                const allMessages = this.state.allMessages.map(m =>
                    m._key === messageKey ? { ...m, text: text, updated_at: Math.floor(Date.now() / 1000) } : m
                );
                const messages = this.state.messages.map(m =>
                    m._key === messageKey ? { ...m, text: text, updated_at: Math.floor(Date.now() / 1000) } : m
                );
                this.update({ allMessages, messages });
            } else {
                    ...
                    if (newThreadMessages.length < initialLength) {
                        ...
                        if (this.state.threadParentMessage) {
                            ...
                            allMessages = allMessages.map(m =>
                                m._key === parentKey ? { ...m, thread_count: newCount } : m
                            );
    ...
    dismissSoundPopup() {
        ...
        this.update({ showSoundPopup: false });
    },
    ...
    // Send browser notification for DMs and @mentions
    sendBrowserNotification(message, type) {
        ...
        const channel = this.state.channels?.find(c => c._id === message.channel_id);
        const senderName = message.sender || 'Someone';
        const channelName = channel ? this.formatChannelName(channel, this.props.currentUser, this.state.users) : 'a channel';

        // Truncate message text for notification body
        const bodyText = message.text ? message.text.substring(0, 100) + (message.text.length > 100 ? '...' : '') : 'New message';
        ...
        try {
            const notification = new Notification(title, {
                ...
                tag: message._key, // Prevent duplicate notifications
                requireInteraction: false,
                silent: true // We play our own sound
            });

            // Click notification to focus and navigate to channel
            notification.onclick = () => {
                window.focus();
                if (channel) {
                    this.selectChannel(channel._key);
                }
                notification.close();
            };
    ...
    async deleteSignal(signalKey) {
        ...
        try {
            await fetch('/talks/delete_signal', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ signal_key: signalKey })
            });
        } catch (e) {
        ...
        try {
            ...
            if (!this.state.activeCall) {
                if (signal.type === 'offer') {
                    ...
                    if (!caller) {
                        caller = { _key: fromUser, firstname: 'Unknown', lastname: 'Caller' };
                    }
                    ...
                    if (this.state.incomingCalls && this.state.incomingCalls.length > 0) {
                        ...
                        if (remaining.length !== this.state.incomingCalls.length) {
                            ...
                            this.update({ incomingCalls: remaining });
            ...
            switch (signal.type) {
                case 'offer':
                    if (data.call_type === 'renegotiation') {
                        ...
                        if (pc) {
                            ...
                            } catch (e) { console.error('Error handling renegotiation:', e); }
                        }
                        return;
                    }

                    await this.setupPeerConnection(fromUser, false, data.call_type);
                    const pc = this.peerConnections[fromUser];
                    if (pc) {
                        await pc.setRemoteDescription(new RTCSessionDescription(data.sdp));
                        const answer = await pc.createAnswer();
                ...
                case 'bye':
                    ...
                    break;
                case 'mute_state':
                    // Track remote peer mute state
                    if (!this.remotePeerStates) this.remotePeerStates = {};
                    if (!this.remotePeerStates[fromUser]) this.remotePeerStates[fromUser] = {};
                    this.remotePeerStates[fromUser].isMuted = data.isMuted;
                    this.updateCallPeers();
                    break;
            }
        } catch (e) {
            console.error('[handleSignal] Error:', e);
            this.update({ lastDebugError: e.message });
        }
    },
    ...
    async acceptCall(call) {
        ...
        try {
            ...
            await this.setupPeerConnection(incoming.from_user, false, incoming.type);
    ...
    async setupPeerConnection(remoteUserKey, isInitiator, callType, isHuddle = false) {
        ...
        // Explicitly add transceivers to ensure media lines (m-lines) are present in SDP
        // This ensures audio works even if the initial stream track is missing or delayed
    ...
    attachLocalStream(retries = 5) {
        this.$nextTick(() => {
            const video = this.root.querySelector('[ref="localVideo"]');
            if (video && this.localStream) {
                console.log('[attachLocalStream] Found video element, attaching stream');
                video.srcObject = this.localStream;
                video.play().catch(e => console.warn('[attachLocalStream] play() failed:', e));
            } else if (retries > 0) {
                // PWA might render slower, retry with increasing delay
                console.log(`[attachLocalStream] Video not ready, retrying... (${retries} left)`);
                setTimeout(() => this.attachLocalStream(retries - 1), 200);
            } else {
                console.error('[attachLocalStream] Failed to attach stream after retries');
            }
        });
    },
    ...
    async hangup() {
        ...
        if (channelId) {
            ...
            if (navigator.sendBeacon) {
                navigator.sendBeacon('/talks/leave_call', blob);
            } else {
    ...
    toggleCallFullscreen() {
        this.update({ isCallFullscreen: !this.state.isCallFullscreen });
    },
    ...
    toggleMute() {
        if (this.localStream) {
            ...
            if (audioTrack) {
                ...
                Object.keys(this.peerConnections).forEach(key => {
                    this.sendSignal(key, 'mute_state', { isMuted });
                });
            }
        }
    },
    ...
    async toggleScreenShare() {
        console.log('[talks-app] toggleScreenShare called');
    ...
    async performSearch(query) {
        ...
        this.update({
            ...
            showThreadSidebar: false, // Close thread
            showMembersPanel: false, // Close members
            threadParentMessage: null,
            threadMessages: [],
            searchQuery: query,
            searchLoading: true,
    ...
    getResultInitials(result) {
        const name = result.sender_firstname && result.sender_lastname
    ...
    getResultSender(result) {
        return result.sender_firstname && result.sender_lastname
            ? result.sender_firstname + ' ' + result.sender_lastname
            : result.sender;
    },
  ...
  template: (
    ...
    '<div expr148="expr148" class="flex h-full bg-[#1A1D21] text-[#D1D2D3] font-sans overflow-hidden"><div expr149="expr149" class="fixed inset-0 bg-black/50 z-40 lg:hidden"></div><talks-sidebar expr150="expr150"></talks-sidebar><main class="flex-1 flex flex-col min-w-0 h-full relative"><talks-header expr151="expr151"></talks-header><talks-messages expr152="expr152"></talks-messages><talks-input expr153="expr153"></talks-input></main><talks-calls expr154="expr154"></talks-calls><div expr155="expr155"></div><div expr169="expr169"><talks-thread expr170="expr170"></talks-thread></div><div expr171="expr171" class="fixed inset-0 z-[9999] bg-black/95 flex items-center justify-center animate-fade-in"></div><div expr177="expr177"></div><div expr182="expr182"></div><div expr186="expr186" class="fixed inset-0 z-50 flex items-center justify-center p-4"></div><div expr206="expr206" class="fixed inset-0 z-[100] flex items-center justify-center p-4"></div><div expr219="expr219" class="fixed inset-0 z-[200] flex items-center justify-center p-4"></div></div>',
    [
      {
      ...
      },
      {
        ...
        evaluate: _scope => _scope.state.isMobile && _scope.state.showMobileSidebar,
              ...
              expressions: [
                {
                  type: expressionTypes.EVENT,
                  name: 'onclick',
                  evaluate: _scope => _scope.closeMobileSidebar
                }
              ]
            }
          ]
      ...
      },
      {
        ...
        attributes: [
          ...
          },
          {
            ...
            isBoolean: false,
            name: 'channels',
            evaluate: _scope => _scope.state.channels
          },
          {
            type: expressionTypes.ATTRIBUTE,
            isBoolean: false,
          ...
          },
          {
            ...
            isBoolean: false,
            name: 'isMobile',
            evaluate: _scope => _scope.state.isMobile
          },
          {
            type: expressionTypes.ATTRIBUTE,
            isBoolean: false,
            name: 'showMobileSidebar',
            evaluate: _scope => _scope.state.showMobileSidebar
          },
          {
            type: expressionTypes.EVENT,
            name: 'onCloseMobileSidebar',
            evaluate: _scope => _scope.closeMobileSidebar
          }
        ],
      ...
      },
      {
        ...
        attributes: [
          ...
          {
            type: expressionTypes.EVENT,
            name: 'onToggleMobileSidebar',
            evaluate: _scope => _scope.toggleMobileSidebar
          }
        ],
      ...
      },
      {
        ...
        attributes: [
          ...
          },
          {
            ...
            isBoolean: false,
            name: 'local-stream',
            evaluate: _scope => _scope.getLocalStream()
          },
          {
            type: expressionTypes.EVENT,
      ...
      },
      {
        ...
        template: template(
          '<div class="flex items-center justify-between p-4 border-b border-gray-700"><div class="flex items-center gap-2"><i class="fas fa-search text-indigo-400"></i><span class="text-white font-semibold">Search Results</span><span expr156="expr156" class="text-gray-500 text-sm"> </span></div><button expr157="expr157" class="text-gray-400 hover:text-white p-1 rounded hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div expr158="expr158" class="px-4 py-2 bg-gray-800/50 border-b border-gray-700"></div><div expr160="expr160" class="flex-1 flex items-center justify-center"></div><div expr161="expr161" class="flex-1 flex items-center justify-center"></div><div expr162="expr162" class="flex-1 overflow-y-auto custom-scrollbar"></div>',
          [
            {
              expressions: [
                {
                  ...
                  isBoolean: false,
                  name: 'class',
                  evaluate: _scope => 'bg-[#1A1D21] border-l border-gray-700 z-10 flex flex-col h-full flex-shrink-0 ' + (_scope.state.isMobile ? 'fixed inset-y-0 right-0 w-full' : 'w-96')
                }
              ]
            },
            {
              ...
              expressions: [
                {
                  ...
                  evaluate: _scope => [
                    '(',
                    _scope.state.searchResults.length,
                    ')'
                  ].join(
                    ''
                  )
            ...
            },
            {
              ...
              template: template(
                '<span class="text-gray-400 text-sm">Searching for: </span><span expr159="expr159" class="text-indigo-400 font-medium"> </span>',
                [
                  {
            ...
            },
            {
              ...
              template: template(
                '<div class="flex flex-col items-center gap-2"><i class="fas fa-spinner fa-spin text-2xl text-indigo-500"></i><span class="text-gray-400 text-sm">Searching...</span></div>',
                []
              )
            },
            {
              ...
              template: template(
                '<div class="flex flex-col items-center gap-2 text-gray-400"><i class="fas fa-search text-4xl text-gray-600"></i><span>No results found</span><span class="text-sm text-gray-500">Try a different search term</span></div>',
                []
              )
            },
            {
              ...
              template: template(
                '<div expr163="expr163" class="p-4 border-b border-gray-700/50 hover:bg-gray-800/50 cursor-pointer transition-colors\n                        group"></div>',
                [
                  {
                    ...
                    template: template(
                      '<div class="flex items-center gap-2 mb-2"><span expr164="expr164" class="text-xs px-2 py-0.5 rounded bg-indigo-600/30 text-indigo-300"> </span><span expr165="expr165" class="text-xs text-gray-500"> </span></div><div class="flex items-start gap-3"><div expr166="expr166" class="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0"> </div><div class="flex-1 min-w-0"><div expr167="expr167" class="text-sm text-gray-200 font-medium truncate"> </div><div expr168="expr168" class="text-sm text-gray-400 line-clamp-2 group-hover:text-gray-300"> </div></div></div>',
                      [
                        {
                          expressions: [
                            {
                          ...
                          expressions: [
                            {
                              ...
                              evaluate: _scope => _scope.getChannelLabel(
                                _scope.result
                              )
                            }
                          ]
                          ...
                          expressions: [
                            {
                              ...
                              evaluate: _scope => _scope.formatSearchTime(
                                _scope.result.timestamp
                              )
                            }
                          ]
                        },
                          ...
                          expressions: [
                            {
                              ...
                              evaluate: _scope => _scope.getResultPreview(
                                _scope.result
                              )
                            }
                          ]
        ...
        expressions: [
          {
            ...
            isBoolean: false,
            name: 'class',
            evaluate: _scope => 'flex-shrink-0 h-full ' + (_scope.state.showThreadSidebar ? '' : 'hidden' ) + ' ' + (_scope.state.isMobile ? 'fixed inset-y-0 right-0 w-4/5 max-w-md bg-[#1A1D21] z-30 shadow-[-4px_0_20px_rgba(0,0,0,0.5)]' : 'w-96')
          }
        ]
      },
      {
      ...
      },
      {
        ...
        template: template(
          '<div expr172="expr172" class="flex flex-col max-w-[90vw] max-h-[90vh]"><img expr173="expr173" class="max-w-full max-h-[80vh] object-contain rounded-lg shadow-2xl"/><div class="flex items-center justify-between mt-4 px-1"><div expr174="expr174" class="text-white/70 text-sm truncate max-w-[60%]"> </div><div class="flex items-center gap-2"><a expr175="expr175" class="flex items-center gap-2 px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors text-sm"><i class="fas fa-download"></i> Download\n                            </a><button expr176="expr176" class="flex items-center gap-2 px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors text-sm"><i class="fas fa-times"></i> Close\n                            </button></div></div></div>',
          [
            {
              expressions: [
                {
      ...
      },
      {
        ...
        template: template(
          '<div expr178="expr178" class="fixed inset-0 z-[-1]"></div><div class="text-xs text-gray-500 uppercase font-bold mb-2">Smileys</div><div class="flex flex-wrap gap-1 mb-3"><button expr179="expr179" class="p-1.5 text-xl hover:bg-gray-700 rounded transition-colors"></button></div><div class="text-xs text-gray-500 uppercase font-bold mb-2">Gestures</div><div class="flex flex-wrap gap-1 mb-3"><button expr180="expr180" class="p-1.5 text-xl hover:bg-gray-700 rounded transition-colors"></button></div><div class="text-xs text-gray-500 uppercase font-bold mb-2">Objects</div><div class="flex flex-wrap gap-1 mb-3"><button expr181="expr181" class="p-1.5 text-xl hover:bg-gray-700 rounded transition-colors"></button></div>',
          [
            {
              expressions: [
                {
                  ...
                  isBoolean: false,
                  name: 'class',
                  evaluate: _scope => 'fixed bg-gray-900 border border-gray-700 rounded-lg shadow-xl z-[9990] animate-fade-in overflow-y-auto custom-scrollbar ' + (_scope.state.isMobile ? 'p-2 bottom-0 left-0 right-0 max-h-[50vh] rounded-t-xl' : 'p-3')
                },
                {
                  type: expressionTypes.ATTRIBUTE,
                  isBoolean: false,
        ...
        template: template(
          ...
          [
              ...
              expressions: [
                {
                  ...
                  isBoolean: false,
                  name: 'class',
                  evaluate: _scope => 'fixed bg-[#222529] border border-gray-700 rounded-lg shadow-2xl z-[9995] overflow-hidden animate-fade-in ' + (_scope.state.isMobile ? 'left-4 right-4 bottom-20 max-h-48' : 'w-64')
                },
                {
                  type: expressionTypes.ATTRIBUTE,
                  isBoolean: false,
            ...
            },
            {
              ...
              template: template(
                '<div expr184="expr184" class="w-6 h-6 rounded-md bg-indigo-500 flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0"> </div><span expr185="expr185" class="text-sm truncate font-medium"> </span>',
                [
                  {
                    expressions: [
                      {
      ...
      },
      {
        ...
        template: template(
          '<div expr187="expr187" class="absolute inset-0 bg-black/60 backdrop-blur-sm"></div><div expr188="expr188"><div class="p-6"><h2 class="text-xl font-bold text-white mb-2">Create a Channel</h2><p class="text-gray-400 text-sm mb-6">Channels are where your team communicates. They\'re best\n                            when organized around a topic.</p><div class="mb-4"><label class="block text-gray-300 text-sm font-bold mb-2">Name</label><div class="relative"><span class="absolute left-3 top-2.5 text-gray-500">#</span><input expr189="expr189" ref="newChannelInput" type="text" class="w-full bg-[#222529] border border-gray-700 text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block pl-8 p-2.5" placeholder="e.g. plan-budget"/></div><p class="mt-2 text-xs text-gray-500">Lowercase, numbers, and hyphens only.</p></div><div class="mb-4"><label class="flex items-center cursor-pointer select-none"><div class="relative"><input expr190="expr190" type="checkbox" class="sr-only"/><div expr191="expr191"></div><div expr192="expr192"></div></div><div class="ml-3 text-sm font-medium text-gray-300 flex items-center">\n                                    Private Channel <i class="fas fa-lock text-xs ml-2 text-gray-500"></i></div></label><p class="text-xs text-gray-500 mt-1 ml-14">Only invited members can view this channel.</p></div><div expr193="expr193" class="mb-4 animate-fade-in"></div><div expr203="expr203" class="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 text-sm"></div></div><div class="px-6 py-4 bg-[#222529] border-t border-gray-700 flex justify-end gap-3"><button expr204="expr204" class="px-4 py-2 text-sm\n                            font-medium text-gray-300 hover:text-white transition-colors">Cancel</button><button expr205="expr205" class="px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"> </button></div></div>',
          [
            {
...
showCreateChannelModal: false })
              ...
              expressions: [
                {
                  ...
                  isBoolean: false,
                  name: 'class',
                  evaluate: _scope => 'relative bg-[#1A1D21] border border-gray-700 rounded-xl shadow-2xl w-full overflow-hidden animate-fade-in-up ' + (_scope.state.isMobile ? 'max-w-full mx-4 my-4 h-full max-h-[90vh]' : 'max-w-md')
                }
              ]
            },
            {
            ...
            },
            {
              ...
              template: template(
                '<label class="block text-gray-300 text-sm font-bold mb-2">Add Members</label><div class="bg-[#222529] border border-gray-700 rounded-lg p-2"><div expr194="expr194" class="flex flex-wrap gap-2 mb-2"><span expr195="expr195" class="bg-blue-500/20 text-blue-300 text-xs px-2 py-1 rounded flex items-center border border-blue-500/30"></span></div><input expr197="expr197" type="text" ref="createChannelMemberInput" placeholder="Search users..." class="w-full bg-transparent text-sm text-gray-200 focus:outline-none placeholder-gray-500 py-1"/><div expr198="expr198" class="mt-2 border-t border-gray-700\n                                    pt-2 max-h-32 overflow-y-auto custom-scrollbar"><div expr199="expr199" class="flex items-center p-2 hover:bg-white/5\n                                        rounded cursor-pointer"></div></div></div>',
                [
                  {
                  ...
                  },
                  {
                    ...
                    template: template(
                      '<div expr200="expr200" class="w-8 h-8 rounded bg-gradient-to-br from-indigo-500 to-purple-600 text-xs flex items-center justify-center text-white font-bold mr-3 flex-shrink-0"> </div><div class="flex-1 min-w-0"><div expr201="expr201" class="text-gray-300 text-sm font-medium truncate"> </div><div expr202="expr202" class="text-gray-500 text-xs truncate"> </div></div>',
                      [
                        {
                          expressions: [
                            {
      ...
      },
      {
        ...
        template: template(
          '<div expr207="expr207" class="absolute inset-0 bg-black/80 backdrop-blur-sm transition-opacity"></div><div expr208="expr208"><div class="p-4 border-b border-gray-700 flex flex-col gap-3"><div class="flex items-center justify-between"><h2 class="text-lg font-bold text-white">New Conversation</h2><button expr209="expr209" class="text-gray-400 hover:text-white transition-colors"><i class="fas fa-times"></i></button></div><div class="relative"><i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-500"></i><input expr210="expr210" type="text" placeholder="Find people..." class="w-full bg-[#0D0B0E] text-gray-200 rounded-lg pl-10 pr-4 py-2 border border-gray-700 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all placeholder-gray-600" ref="dmFilterInput"/></div></div><div class="overflow-y-auto custom-scrollbar p-2"><div expr211="expr211" class="flex items-center\n                            gap-3 p-3 hover:bg-white/5 rounded-lg cursor-pointer transition-colors group"></div><div expr217="expr217" class="p-8 text-center text-gray-500 flex flex-col items-center"></div></div></div>',
          [
            {
              ...
              expressions: [
                {
                  ...
                  isBoolean: false,
                  name: 'class',
                  evaluate: _scope => 'relative bg-[#1A1D21] rounded-xl border border-gray-700 shadow-2xl overflow-hidden animate-fade-in-up flex flex-col ' + (_scope.state.isMobile ? 'w-full max-w-full mx-4 my-4 h-full max-h-[90vh]' : 'w-full max-w-lg max-h-[80vh]')
                }
              ]
            },
            {
            ...
            },
            {
              ...
              template: template(
                '<div class="relative"><div expr212="expr212" class="w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-sm font-bold text-white shadow-lg"> </div><div expr213="expr213"></div></div><div class="flex-1 min-w-0"><div class="flex items-center justify-between"><span expr214="expr214" class="text-gray-200 font-medium group-hover:text-white transition-colors truncate"> </span><span expr215="expr215" class="text-xs text-gray-500 italic"></span></div><div expr216="expr216" class="text-xs text-gray-500 truncate"> </div></div><i class="fas fa-chevron-right text-gray-600 group-hover:text-gray-400 transition-colors"></i>',
                [
                  {
                    expressions: [
                      {
            ...
            },
            {
              ...
              template: template(
                '<i class="fas fa-user-slash text-4xl mb-3 opacity-50"></i><p expr218="expr218"> </p>',
                [
                  {
      ...
      },
      {
        ...
        template: template(
          '<div class="absolute inset-0 bg-black/80 backdrop-blur-sm"></div><div expr220="expr220"><div class="w-16 h-16 bg-blue-600/20 text-blue-400 rounded-full flex items-center justify-center mx-auto mb-6"><i class="fas fa-volume-up text-3xl"></i></div><h2 class="text-xl font-bold text-white mb-2">Enable Sounds</h2><p class="text-gray-400 mb-8">Talks uses sounds for new messages and calls. Please click Enable to\n                        ensure you don\'t miss anything.</p><div class="flex gap-3 justify-center"><button expr221="expr221" class="px-5 py-2.5 rounded-lg bg-blue-600 text-white hover:bg-blue-500 transition-colors font-medium shadow-lg shadow-blue-900/20">Enable\n                            Sounds</button></div></div>',
          [
            {
              ...
              expressions: [
                {
                  ...
                  isBoolean: false,
                  name: 'class',
                  evaluate: _scope => 'relative bg-[#1A1D21] border border-gray-700 rounded-xl shadow-2xl overflow-hidden animate-fade-in-up text-center ' + (_scope.state.isMobile ? 'w-full max-w-full mx-4 my-4 p-6' : 'w-full max-w-sm p-8')
                }
              ]
            },
            {
