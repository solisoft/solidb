...
export default {
  ...
  exports: {
    ...
    addColumn() {
      const columns = [...this.state.columns, { name: '', data_type: 'String', nullable: false, indexed: false }]
      this.update({ columns })
    },
    ...
    updateColumnType(index, e) {
      const columns = [...this.state.columns]
      columns[index] = { ...columns[index], data_type: e.target.value }
      this.update({ columns })
    },

    updateColumnNullable(index, e) {
      const columns = [...this.state.columns]
      columns[index] = { ...columns[index], nullable: e.target.checked }
      this.update({ columns })
    },

    handleCompressionChange(e) {
      this.update({ compression: e.target.value })
    },
    ...
    async handleSubmit(e) {
      ...
      if (this.state.collectionType === 'columnar') {
        ...
        if (validColumns.length === 0) {
          this.update({ error: 'At least one column is required for columnar collections', loading: false })
          return
        }
        ...
        try {
          ...
          if (response.ok) {
            ...
            this.update({ error: error.error || 'Failed to create columnar collection', loading: false })
          }
        } catch (error) {
  ...
  template: (
    ...
    '<div expr101="expr101" id="modalBackdrop" class="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm transition-all duration-300 ease-out opacity-0 hidden"><div class="absolute inset-0 bg-black/50 transition-opacity duration-300"></div><div expr102="expr102" id="modalContent" class="relative bg-gray-900/80 backdrop-blur-xl rounded-xl shadow-2xl w-full max-w-md flex flex-col border border-white/10 overflow-hidden transform transition-all duration-300 ease-out scale-95 opacity-0 ring-1 ring-white/10"><div class="px-6 py-4 border-b border-gray-700/50 bg-gray-800/50 backdrop-blur-md sticky top-0 z-10"><h3 class="text-xl font-semibold text-white tracking-tight">Create New Collection</h3></div><div class="p-6"><div expr103="expr103" class="mb-4 p-3 bg-red-900/20 border border-red-500/50 rounded"></div><form expr105="expr105"><div class="mb-4"><label class="block text-sm font-medium text-gray-300 mb-2">Collection Name</label><input expr106="expr106" type="text" ref="nameInput" required pattern="[a-zA-Z0-9_]+" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:bg-gray-900 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors" placeholder="e.g., users, products"/><p class="mt-1 text-xs text-gray-500">Only letters, numbers, and underscores allowed</p></div><div class="mb-4"><label class="block text-sm font-medium text-gray-300 mb-2">Collection Type</label><div class="grid grid-cols-2 gap-3"><button expr107="expr107" type="button" data-type="document"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>\n                Document\n              </button><button expr108="expr108" type="button" data-type="edge"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>\n                Edge\n              </button><button expr109="expr109" type="button" data-type="timeseries"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>\n                Time Series\n              </button><button expr110="expr110" type="button" data-type="blob"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>\n                Blob\n              </button><button expr111="expr111" type="button" data-type="columnar"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h4v14H4V6zm6 0h4v14h-4V6zm6 0h4v14h-4V6z"/></svg>\n                Columnar\n              </button></div><p expr112="expr112" class="mt-2 text-xs text-fuchsia-400"></p><p expr113="expr113" class="mt-2 text-xs text-amber-400"></p><p expr114="expr114" class="mt-2 text-xs text-cyan-400"></p><p expr115="expr115" class="mt-2 text-xs text-emerald-400"></p></div><div expr116="expr116" class="mb-6 border-t border-gray-700/50 pt-4"></div><div expr125="expr125" class="mb-6 border-t border-gray-700/50 pt-4"></div><div class="flex justify-end space-x-3 pt-2"><button expr131="expr131" type="button" class="px-4 py-2 text-sm font-medium text-gray-400 hover:text-white transition-colors hover:bg-gray-800/50 rounded-lg">\n              Cancel\n            </button><button expr132="expr132" type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg shadow-lg shadow-indigo-600/20 transition-all disabled:opacity-50 disabled:shadow-none"> </button></div></form></div></div></div>',
    [
      {
      ...
      },
      {
        ...
        template: template(
          '<div class="flex items-start"><svg class="h-5 w-5 text-red-400 mr-2 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><p expr104="expr104" class="text-sm text-red-300"> </p></div>',
          [
            {
      ...
      },
      {
        ...
        template: template(
          '<div class="flex items-center justify-between mb-3"><label class="block text-sm font-medium text-gray-300">Column Definitions</label><button expr117="expr117" type="button" class="text-xs px-2 py-1 bg-emerald-600/20 text-emerald-400 rounded hover:bg-emerald-600/30 transition-colors">\n                + Add Column\n              </button></div><div class="space-y-2 max-h-48 overflow-y-auto"><div expr118="expr118" class="flex items-center gap-2 p-2 bg-gray-800/50 rounded-lg border border-gray-700/50"></div></div><p expr123="expr123" class="text-xs text-gray-500 mt-2"></p><div class="mt-4"><label class="block text-xs font-medium text-gray-400 mb-1">Compression</label><select expr124="expr124" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-100 text-sm focus:outline-none focus:border-emerald-500"><option value="lz4">LZ4 (Recommended)</option><option value="none">None</option></select></div>',
          [
            {
            ...
            },
            {
              ...
              template: template(
                '<input expr119="expr119" type="text" placeholder="Column name" class="flex-1 px-2 py-1.5 bg-gray-900 border border-gray-600 rounded text-gray-100 text-xs focus:outline-none focus:border-emerald-500"/><select expr120="expr120" class="px-2 py-1.5 bg-gray-900 border border-gray-600 rounded text-gray-100 text-xs focus:outline-none focus:border-emerald-500"><option value="String">String</option><option value="Int64">Int64</option><option value="Float64">Float64</option><option value="Bool">Bool</option><option value="Timestamp">Timestamp</option></select><label class="flex items-center gap-1 text-xs text-gray-400"><input expr121="expr121" type="checkbox" class="h-3 w-3 text-emerald-500 border-gray-600 rounded bg-gray-800"/>\n                  Null\n                </label><button expr122="expr122" type="button" class="p-1 text-red-400 hover:text-red-300 hover:bg-red-900/20 rounded transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>',
                [
                  {
      ...
      },
      {
        ...
        template: template(
          '<div class="flex items-center mb-4"><input expr126="expr126" id="enableSharding" type="checkbox" class="h-4 w-4 text-indigo-500 focus:ring-indigo-500 border-gray-600 rounded bg-gray-800 transition-colors"/><label for="enableSharding" class="ml-2 block text-sm text-gray-300">\n                Enable Sharding & Replication\n              </label></div><div expr127="expr127" class="space-y-4 pl-6 border-l-2 border-gray-700/50"></div>',
          [
            {
