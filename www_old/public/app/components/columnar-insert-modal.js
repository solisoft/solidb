...
var columnarInsertModal = {
  ...
  template: (template, expressionTypes, bindingTypes, getComponent) => template('<div expr573="expr573" id="modalBackdrop" class="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm transition-all duration-300 ease-out opacity-0 hidden"><div class="absolute inset-0 bg-black/50 transition-opacity duration-300"></div><div expr574="expr574" id="modalContent" class="relative bg-gray-900/80 backdrop-blur-xl rounded-xl shadow-2xl w-full max-w-2xl flex flex-col border border-white/10 overflow-hidden transform transition-all duration-300 ease-out scale-95 opacity-0 ring-1 ring-white/10 max-h-[90vh]"><div class="px-6 py-4 border-b border-gray-700/50 bg-gray-800/50 backdrop-blur-md sticky top-0 z-10"><h3 class="text-xl font-semibold text-white tracking-tight">Insert Data</h3><p class="text-sm text-gray-400 mt-1">Add rows to the columnar collection</p></div><div class="p-6 overflow-y-auto"><div expr575="expr575" class="mb-4 p-3 bg-red-900/20 border border-red-500/50 rounded"></div><div class="mb-4"><div class="flex space-x-2"><button expr577="expr577" type="button">\n              Form Entry\n            </button><button expr578="expr578" type="button">\n              JSON (Bulk)\n            </button></div></div><div expr579="expr579"></div><div expr589="expr589"></div></div><div class="px-6 py-4 border-t border-gray-700/50 bg-gray-800/50 flex justify-end space-x-3"><button expr590="expr590" type="button" class="px-4 py-2 text-sm font-medium text-gray-400 hover:text-white transition-colors hover:bg-gray-800/50 rounded-lg">\n          Cancel\n        </button><button expr591="expr591" type="button" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium rounded-lg shadow-lg shadow-emerald-600/20 transition-all disabled:opacity-50"> </button></div></div></div>', [{
    ...
    selector: '[expr575]',
    template: template('<p expr576="expr576" class="text-sm text-red-300"> </p>', [{
    ...
    selector: '[expr579]',
    template: template('<div class="space-y-3"><div expr580="expr580" class="flex items-center gap-3"></div></div><button expr584="expr584" type="button" class="mt-4 w-full px-4 py-2 bg-emerald-600/20 text-emerald-400 rounded-lg hover:bg-emerald-600/30 transition-colors text-sm"> </button><div expr585="expr585" class="mt-4 max-h-40 overflow-y-auto"></div>', [{
      type: bindingTypes.EACH,
      getKey: null,
      condition: null,
      template: template('<label expr581="expr581" class="w-32 text-sm font-medium text-gray-300 flex-shrink-0"> <span expr582="expr582" class="text-gray-500 text-xs ml-1"> </span></label><input expr583="expr583" type="text" class="flex-1 px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-100 text-sm focus:outline-none focus:border-emerald-500"/>', [{
      ...
      selector: '[expr585]',
      template: template('<div class="text-xs text-gray-500 mb-2">Queued rows:</div><div expr586="expr586" class="flex items-center justify-between p-2 bg-gray-800/50 rounded mb-1 text-xs"></div>', [{
        type: bindingTypes.EACH,
        getKey: null,
        condition: null,
        template: template('<span expr587="expr587" class="text-gray-400 truncate flex-1"> </span><button expr588="expr588" class="text-red-400 hover:text-red-300 ml-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>', [{
