...
fn bench_transactions(client: &Client) {
    ...
    for _ in 0..SMALL {
        let begin_url = format!(
            "{}/_api/database/{}/transaction/begin",
            SERVER_URL, DATABASE
        );
        let response = client
            .post(&begin_url)
    ...
    for i in 0..SMALL {
        ...
        let doc_url = format!(
            "{}/_api/database/{}/document/bench_http",
            SERVER_URL, DATABASE
        );
        client
            .post(&doc_url)
    ...
    for i in 0..SMALL {
        ...
        let doc_url = format!(
            "{}/_api/database/{}/document/bench_http/tx_user_{}",
            SERVER_URL, DATABASE, i
        );
        client
            .put(&doc_url)
    ...
    for i in 0..SMALL {
        ...
        client
            ...
            .header("X-Transaction-ID", tx_id)
            .json(
                &json!({"_key": format!("uncommitted_user_{}", i), "name": format!("User {}", i)}),
            )
            .send()
            .expect("Insert failed");
