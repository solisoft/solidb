//! Blob storage handlers for binary file upload and download.
//!
//! This module provides HTTP handlers for managing binary large objects (BLOBs)
//! in SoliDB. It supports:
//! - Multipart file uploads with automatic chunking
//! - Streaming downloads
//! - Distributed chunk storage across cluster nodes
//! - Automatic blob collection creation
...
/// Upload a binary file to a blob collection.
///
/// Accepts multipart form data with a "file" field. The collection will be
/// auto-created as a blob collection if it doesn't exist.
///
/// # Arguments
/// * `state` - Application state containing storage and cluster coordinator
/// * `db_name` - Database name from path
/// * `coll_name` - Collection name from path
/// * `multipart_result` - Multipart form data containing the file
///
/// # Returns
/// JSON document with blob metadata including _key, name, type, size, chunks
        ...
        Ok(coll) => {
            ...
            if coll.get_type() != "blob" {
                return Err(DbError::BadRequest(format!(
                    "Collection '{}' is not a blob collection. Please create it as a blob collection first.",
                    coll_name
                )));
            }
            coll
...
/// Download a binary file from a blob collection.
///
...
/// * `db_name` - Database name from path
/// * `coll_name` - Collection name from path
/// * `key` - Blob document key from path
///
/// # Returns
...
/// Chunks are replicated to multiple nodes using round-robin distribution.
pub(crate) async fn distribute_blob_chunks_across_cluster(
...
///
/// Tries each node in the cluster to find the requested chunk.
/// Returns None if the chunk is not found on any node.
pub(crate) async fn fetch_blob_chunk_from_cluster(
    coordinator: &crate::sharding::coordinator::ShardCoordinator,
    db_name: &str,
    coll_name: &str,
    blob_key: &str,
