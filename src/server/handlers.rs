use axum::{
    ...
    extract::{
        ...
        Multipart, Path, Query as AxumQuery, State,
    },
                        ...
                        if let Some(table) = coordinator.get_shard_table(&db_name, &name) {
                            ...
                            for shard_id in 0..config.num_shards {
                                ...
                                if let Some(assignment) = table.assignments.get(&shard_id) {
                                    ...
                                    let is_primary = assignment.primary_node == local_id
                                    ...
                                    if is_primary || is_replica {
                                        ...
                                        if found {
                                            ...
                                            tracing::warn!(
                                                "Could not get count for shard {} from any node",
                                                shard_id
                                            );
                                        }
                                    }
            ...
            if first_char == b'{' {
                                    ...
                                    if batch_docs.len() >= 1000 {
                                        if is_sharded && state.shard_coordinator.is_some() {
                                            ...
                                            match collection.insert_batch(batch_docs.clone()) {
                                                Ok(inserted) => {
                                                    if let Err(e) =
                                                        collection.index_documents(&inserted)
                                                    {
                                                        tracing::error!(
                ...
                if !batch_docs.is_empty() {
                    if is_sharded && state.shard_coordinator.is_some() {
                        ...
                        match coordinator
                            .insert_batch(&db_name, &coll_name, config, docs_to_insert)
                            .await
                        {
                            Ok((successes, failures)) => {
                                imported_count += successes;
                ...
                let text = String::from_utf8(buffer)
                    .map_err(|e| DbError::BadRequest(format!("Invalid UTF-8: {}", e)))?;

                let docs: Vec<Value> = if first_char == b'[' {
                    serde_json::from_str(&text)
                        .map_err(|e| DbError::BadRequest(format!("Invalid JSON Array: {}", e)))?
                } else {
                    // CSV
...
pub struct HybridSearchRequest {
    ...
    pub limit: usize,
    #[serde(default)]
    pub fusion: String, // "weighted" (default) or "rrf"
}
...
/// Hybrid search combining vector similarity with fulltext search
pub async fn hybrid_search(
    ...
    if fusion_method == "rrf" {
        ...
        for doc_key in all_doc_keys {
            let vec_score_norm = vector_scores.get(&doc_key).copied();
            let txt_score_norm = text_scores.get(&doc_key).copied();

            let mut sources = Vec::new();
            let mut combined_score = 0.0;

            if vec_score_norm.is_some() {
                combined_score += vec_score_norm.unwrap() * req.vector_weight;
                sources.push("vector".to_string());
            }
            if txt_score_norm.is_some() {
                combined_score += txt_score_norm.unwrap() * req.text_weight;
    ...
    combined_results.truncate(req.limit);

    // Step 7: Build response with documents
    let search_results: Vec<HybridSearchResult> = combined_results
        .into_iter()
        .map(|(doc_key, score, vec_score, txt_score, sources)| {
            let document = collection.get(&doc_key).ok().map(|doc| doc.to_value());
            HybridSearchResult {
                doc_key,
                score,
