...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..iterations {
        ...
        client.insert(db, col, Some(&format!("base_{}", i)), doc).await?;
    }
    ...
    let start = Instant::now();
    client.begin_transaction(db, Some(IsolationLevel::ReadCommitted)).await?;
    for i in 0..iterations {
        let doc = json!({ "id": i, "val": "bulk" });
        client.insert(db, col, Some(&format!("bulk_{}", i)), doc).await?;
    }
    client.commit().await?;
    let duration = start.elapsed();
    let bulk_ops = iterations as f64 / duration.as_secs_f64();
    println!("Bulk TX (1 TX, {} ops): {:.2} ops/sec", iterations, bulk_ops);
    ...
    for i in 0..iterations {
        ...
        client.insert(db, col, Some(&format!("many_{}", i)), doc).await?;
        client.commit().await?;
    }
    let duration = start.elapsed();
    let many_ops = iterations as f64 / duration.as_secs_f64();
    println!("Many TXs ({} TXs, 1 op each): {:.2} ops/sec", iterations, many_ops);
    ...
    if bulk_ops > baseline_ops {
        println!("  - Bulk Transactions IMPROVED performance by {:.1}%", (bulk_ops - baseline_ops) / baseline_ops * 100.0);
    } else {
        println!("  - Bulk Transactions DECREASED performance by {:.1}%", (baseline_ops - bulk_ops) / baseline_ops * 100.0);
    }
    
    println!("  - Individual Transactions overhead: {:.1}% slowdown", (baseline_ops - many_ops) / baseline_ops * 100.0);
