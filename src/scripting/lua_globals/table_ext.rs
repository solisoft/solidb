...
pub fn setup_table_lib_extensions(lua: &Lua) -> Result<(), DbError> {
    ...
    table_lib.set("keys", keys_fn).map_err(|e| {
        DbError::InternalError(format!("Failed to set table.keys: {}", e))
    })?;
    ...
    table_lib.set("map", map_fn).map_err(|e| {
        DbError::InternalError(format!("Failed to set table.map: {}", e))
    })?;
