...
pub fn setup_crypto_globals(lua: &Lua) -> Result<(), DbError> {
    ...
    let random_string_fn = lua
        ...
        .map_err(|e| DbError::InternalError(format!("Failed to create random_string function: {}", e)))?;
    crypto.set("random_string", random_string_fn).map_err(|e| DbError::InternalError(format!("Failed to set random_string: {}", e)))?;
    ...
    let jwt_encode_fn = lua
        .create_function(move |lua, (claims, secret): (LuaValue, String)| -> Result<String, mlua::Error> {
            let json_claims = lua_to_json_value(lua, claims)?;
            let token = encode(&Header::default(), &json_claims, &EncodingKey::from_secret(secret.as_bytes()))
    ...
    let jwt_decode_fn = lua
        .create_function(move |lua, (token, secret): (String, String)| -> Result<mlua::Value, mlua::Error> {
