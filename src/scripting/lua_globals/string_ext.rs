...
pub fn setup_string_extensions(lua: &Lua) -> Result<(), DbError> {
    ...
    string_table.set("regex_replace", regex_replace_fn).map_err(|e| {
    ...
    string_table.set("trim", trim_fn).map_err(|e| {
        DbError::InternalError(format!("Failed to set string.trim: {}", e))
    })?;
