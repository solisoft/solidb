...
pub fn create_set_cookie_function(lua: &Lua) -> LuaResult<Function> {
    lua.create_function(move |_lua, (name, value, options): (String, String, Option<LuaValue>)| {
        ...
        if let Some(opts) = options {
            if let LuaValue::Table(t) = opts {
                ...
                if let Ok(expires) = t.get::<String>("expires") {
                    ...
                    } else if let Ok(datetime) = OffsetDateTime::parse(&expires, &format_description::well_known::Rfc3339) {
                        cookie.set_expires(datetime);
                    }
                }
...
pub fn create_cache_function(lua: &Lua) -> LuaResult<Function> {
    lua.create_function(move |lua, (key, value, ttl): (String, LuaValue, Option<u64>)| {
...
pub fn create_response_html_function(_lua: &Lua) -> LuaResult<Function> {
    ...
    lua_ref.create_function(move |lua, content: String| {
        ...
        Ok(LuaValue::String(lua.create_string(&format!("HTML_RESPONSE:{}", content)).unwrap()))
    })
}
...
mod tests {
    ...
    fn test_cache_function() {
        ...
        let result: Result<bool, _> = cache_fn.call(("test_key".to_string(), LuaValue::Table(data), Some(60)));
