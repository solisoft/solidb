...
pub fn create_error_function(lua: &Lua) -> LuaResult<Function> {
    lua.create_function(move |_, (message, code): (String, Option<u16>)| {
        ...
        Err::<LuaValue, mlua::Error>(mlua::Error::RuntimeError(format!("ERROR:{}:{}", error_code, message)))
    })
}
...
pub fn create_try_function(lua: &Lua) -> LuaResult<Function> {
    lua.create_async_function(move |lua, (try_fn, catch_fn): (Function, Option<Function>)| {
        async move {
            match try_fn.call_async::<LuaValue>(LuaValue::Nil).await {
                Ok(result) => Ok(result),
                Err(e) => {
                    if let Some(catch) = catch_fn {
                        match catch.call_async::<LuaValue>(LuaValue::String(lua.create_string(e.to_string())?)).await {
                            Ok(catch_result) => Ok(catch_result),
                            Err(catch_error) => Err(mlua::Error::RuntimeError(format!("Catch function failed: {}", catch_error))),
                        }
                    } else {
                        Err(e)
                    }
...
pub fn create_validate_condition_function(lua: &Lua) -> LuaResult<Function> {
    lua.create_function(move |_, (condition, message, code): (bool, String, Option<u16>)| {
        if !condition {
            let error_code = code.unwrap_or(400);
            return Err(mlua::Error::RuntimeError(format!("VALIDATION:{}:{}", error_code, message)));
        }
        Ok(true)
    })
}
...
pub fn create_check_permissions_function(lua: &Lua) -> LuaResult<Function> {
    lua.create_function(move |lua, (user_json, required_json): (LuaValue, LuaValue)| {
        let user = lua_to_json_value(lua, user_json)?;
        let required = lua_to_json_value(lua, required_json)?;
        
        if let (JsonValue::Object(user_obj), JsonValue::Array(required_array)) = (&user, &required) {
            let user_permissions = if let Some(JsonValue::Array(perm_array)) = user_obj.get("permissions") {
                perm_array.iter()
                ...
                if let Some(required_perm) = perm.as_str() {
                    if !user_permissions.contains(required_perm) {
                        return Err(mlua::Error::RuntimeError(
                            format!("PERMISSION_DENIED:Missing permission: {}", required_perm)
                        ));
                    }
                }
            }
            ...
            Err(mlua::Error::RuntimeError("Invalid user or permissions format".to_string()))
        }
    })
}
...
pub fn create_rate_limit_function(lua: &Lua) -> LuaResult<Function> {
    ...
    lua.create_function(move |_, (identifier, max_requests, window_seconds): (String, u32, u64)| {
        if limiter.check_limit(&identifier, max_requests, window_seconds) {
            Ok(true)
        } else {
            Err(mlua::Error::RuntimeError(format!("RATE_LIMIT:{}:Too many requests", 429)))
        }
    })
}
...
pub fn create_timeout_function(lua: &Lua) -> LuaResult<Function> {
    lua.create_async_function(move |_lua, (timeout_ms, func): (u64, Function)| {
        async move {
            match tokio::time::timeout(
                ...
                Err(_) => Err(mlua::Error::RuntimeError(
                    format!("TIMEOUT:Operation timed out after {}ms", timeout_ms)
                )),
            }
        }
    })
...
pub fn create_retry_function(lua: &Lua) -> LuaResult<Function> {
    lua.create_async_function(move |_lua, (max_attempts, delay_ms, func): (u32, u64, Function)| {
                ...
                match func_clone.call_async::<LuaValue>(LuaValue::Nil).await {
                    ...
                    Err(e) => {
                        if attempt == max_attempts {
                            return Err(mlua::Error::RuntimeError(
                                format!("RETRY_FAILED:All {} attempts failed. Last error: {}", max_attempts, e)
                            ));
                        }
                        // Wait before retry
...
pub fn create_fallback_function(lua: &Lua) -> LuaResult<Function> {
    lua.create_async_function(move |_lua, (primary_fn, fallback_fn): (Function, Function)| {
        async move {
            match primary_fn.call_async::<LuaValue>(LuaValue::Nil).await {
                Ok(result) => Ok(result),
                Err(primary_error) => {
                    // Log the primary error but use fallback
                    tracing::warn!("Primary function failed, using fallback: {}", primary_error);
...
pub fn create_validate_input_function(lua: &Lua) -> LuaResult<Function> {
    lua.create_function(move |lua, (input_value, rules_value): (LuaValue, LuaValue)| {
                ...
                if let JsonValue::Object(rules_map) = field_rules {
                    ...
                    if let Some(JsonValue::Bool(true)) = rules_map.get("required") {
                        if input.get(&field_name).is_none() {
                            return Err(mlua::Error::RuntimeError(
                                format!("VALIDATION:Required field missing: {}", field_name)
                            ));
                        }
                    }
                    ...
                    if let Some(expected_type) = rules_map.get("type") {
                        if let (Some(expected_str), Some(field_value)) = (expected_type.as_str(), input.get(&field_name)) {
                            ...
                            if !type_matches {
                                return Err(mlua::Error::RuntimeError(
                                    format!("VALIDATION:Field {} must be of type {}", field_name, expected_str)
                                ));
                            }
                        }
                    }
                    ...
                    if let (Some(field_value), Some(JsonValue::Number(min_val))) = (input.get(&field_name), rules_map.get("min")) {
                        if let Some(num) = field_value.as_f64() {
                            if num < min_val.as_f64().unwrap() {
                                return Err(mlua::Error::RuntimeError(
                                    format!("VALIDATION:Field {} must be at least {}", field_name, min_val)
                                ));
                            }
                        }
                    }
                    
                    if let (Some(field_value), Some(JsonValue::Number(max_val))) = (input.get(&field_name), rules_map.get("max")) {
                        if let Some(num) = field_value.as_f64() {
                            if num > max_val.as_f64().unwrap() {
                                return Err(mlua::Error::RuntimeError(
                                    format!("VALIDATION:Field {} must be at most {}", field_name, max_val)
                                ));
                            }
                        }
                    }
