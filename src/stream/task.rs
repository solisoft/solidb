...
use crate::sdbql::ast::{Query, WindowType, BodyClause};
...
impl StreamTask {
    pub fn new(
        ...
        let (window_type, duration_str) = {
             let window_clause = query.window_clause.as_ref().ok_or(DbError::ExecutionError("Missing WINDOW clause".to_string()))?;
             (window_clause.window_type.clone(), window_clause.duration.clone())
        };
        ...
        let for_clause = query.for_clauses.first().ok_or(DbError::ExecutionError("Missing FOR clause".to_string()))?;
    ...
    pub async fn run(mut self) {
        tracing::info!("Stream {}: Started (Window: {:?})", self.name, self.window_duration);
        ...
        loop {
            ...
            let wait_duration = if now >= self.next_window_end {
                ...
                (self.next_window_end - now).to_std().unwrap_or(std::time::Duration::from_millis(1))
            };
    ...
    async fn process_window(&mut self) -> DbResult<()> {
         tracing::info!("Stream {}: Processing window with {} events", self.name, self.buffer.len());
         ...
         for doc in &self.buffer {
             ...
             for filter_clause in &self.query.filter_clauses {
                 let match_result = executor.evaluate_filter_with_context(&filter_clause.expression, &ctx).unwrap_or(false);
                 if !match_result {
                     keep = false;
                     break;
                 }
                 ...
                 BodyClause::Collect(collect) => {
                     ...
                     if let Some(count_var) = &collect.count_var {
                         ...
                         if collect.group_vars.is_empty() {
                             ...
                             new_ctx.insert(count_var.clone(), Value::Number(serde_json::Number::from(count)));
                             results = vec![new_ctx];
                         }
                     }
                 }
         ...
         if let Some(return_clause) = &self.query.return_clause {
             for ctx in results {
                 let result_val = executor.evaluate_expr_with_context(&return_clause.expression, &ctx)?;
...
fn parse_duration(s: &str) -> DbResult<Duration> {
    if s.ends_with('m') {
        let mins = s[..s.len()-1].parse::<i64>().map_err(|_| DbError::ParseError("Invalid duration".to_string()))?;
        Ok(Duration::minutes(mins))
    } else if s.ends_with('s') {
        let secs = s[..s.len()-1].parse::<i64>().map_err(|_| DbError::ParseError("Invalid duration".to_string()))?;
        Ok(Duration::seconds(secs))
    } else if s.ends_with('h') {
        let hours = s[..s.len()-1].parse::<i64>().map_err(|_| DbError::ParseError("Invalid duration".to_string()))?;
        Ok(Duration::hours(hours))
    } else {
        Err(DbError::ParseError("Unknown duration unit (use s, m, h)".to_string()))
    }
}
