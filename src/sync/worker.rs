...
use super::protocol::{SyncEntry, SyncMessage, NodeStats, Operation};
...
pub enum SyncCommand {
    ...
    /// Add a new peer
    AddPeer { node_id: String, sync_addr: String, http_addr: String },
    /// Remove a peer
...
impl SyncWorker {
    ...
    pub fn with_cluster_manager(mut self, manager: Arc<crate::cluster::manager::ClusterManager>) -> Self {
        self.cluster_manager = Some(manager);
        self
    }

    pub fn with_shard_coordinator(mut self, coordinator: Arc<crate::sharding::ShardCoordinator>) -> Self {
        self.shard_coordinator = Some(coordinator);
        self
    }

    pub fn with_incoming_channel(mut self, rx: mpsc::Receiver<(super::transport::SyncStream, String)>) -> Self {
            ...
            loop {
                match server_clone.accept().await {
                    Ok((stream, addr)) => {
                        ...
                        tokio::spawn(async move {
                            if let Err(e) = Self::handle_connection(stream, addr, pool, state, storage, sync_log, cluster_manager).await {
            ...
            tokio::spawn(async move {
                while let Some((stream, addr)) = rx.recv().await {
                   ...
                   tokio::spawn(async move {
                        ...
                        match super::transport::SyncServer::authenticate_standalone_skip_magic(stream, &keyfile).await {
                            Ok(auth_stream) => {
                                if let Err(e) = Self::handle_connection(auth_stream, addr, pool, state, storage, sync_log, cluster_manager).await {
                                     error!("Connection handler error: {}", e);
                                }
                            }
                            Err(e) => {
    ...
    async fn sync_with_peers(&self) {
        ...
        if let Some(ref manager) = self.cluster_manager {
            ...
            debug!("sync_with_peers: ClusterManager has {} members", cluster_members.len());
            for member in cluster_members {
                ...
                if !already_known {
                    debug!("sync_with_peers: discovered peer {} at {}", member.node.id, member.node.address);
        ...
        for peer in peers {
            debug!("sync_with_peers: syncing with {} at {}", peer.node_id, peer.sync_address);
            ...
            if peer.is_connected || self.pool.connect(&peer.sync_address).await.is_ok() {
                ...
                if let Some(ref mgr) = self.cluster_manager {
                    mgr.state().mark_status(&self.local_node_id, crate::cluster::state::NodeStatus::Syncing);
                }
                ...
                if let Some(ref mgr) = self.cluster_manager {
                    mgr.state().mark_status(&self.local_node_id, crate::cluster::state::NodeStatus::Active);
                }
                
                if pages > 0 {
                    debug!("Synced {} batches from {} (finished, has_more={})", 
    ...
    async fn incremental_sync(&self, peer_addr: &str) -> Result<bool, TransportError> {
        ...
        match response {
            SyncMessage::SyncBatch { entries, has_more, current_sequence, .. } => {
                debug!("incremental_sync: {} entries from {} (seq={}) has_more={}", entries.len(), peer_addr, current_sequence, has_more);
                ...
                while batch_start < entries.len() {
                    ...
                    if matches!(first.operation, Operation::Insert | Operation::Update | Operation::Delete) {
                ...
                let latest_we_have = entries.iter().map(|e| e.sequence).max().unwrap_or(after_seq);
    ...
    async fn apply_batch(&self, entries: &[SyncEntry]) -> Result<(), TransportError> {
        ...
        let is_physical_shard = collection.contains("_s") && 
            collection.chars().last().map(|c| c.is_ascii_digit()).unwrap_or(false);
        if is_physical_shard {
            debug!("apply_batch: Skipping physical shard collection {} (partitioned, not replicated)", collection);
            return Ok(());
        }
                ...
                if let Ok(db) = self.storage.get_database(database) {
                    if let Ok(coll) = db.get_collection(collection) {
                        ...
                        for entry in entries {
                            // Check for duplicate
                            if self.state.is_duplicate(&entry.origin_node, entry.origin_sequence) {
                                continue;
                            }
                        ...
                        if !batch_data.is_empty() {
                            if let Err(e) = coll.upsert_batch(batch_data) {
                                warn!("apply_batch: upsert failed for {}.{}: {}", database, collection, e);
                            }
                        }
                    }
                }
        ...
        for entry in entries {
            self.state.update_origin_sequence(&entry.origin_node, entry.origin_sequence);
        }
    ...
    async fn apply_entry(&self, entry: &SyncEntry) -> Result<(), TransportError> {
        ...
        let is_physical_shard = entry.collection.contains("_s") && 
            entry.collection.chars().last().map(|c| c.is_ascii_digit()).unwrap_or(false);
        if is_physical_shard {
            return Ok(());
        }
        ...
        match entry.operation {
                ...
                if let Some(ref data) = entry.document_data {
                    ...
                    if let Ok(db) = self.storage.get_database(&entry.database) {
                        ...
                        if let Ok(coll) = db.get_collection(&entry.collection) {
                            if let Err(e) = coll.upsert_batch(vec![(entry.document_key.clone(), doc)]) {
                                warn!("apply_entry: upsert failed for {}: {}", entry.document_key, e);
                            }
                        }
                    }
                }
            ...
            Operation::CreateCollection => {
                if let Ok(db) = self.storage.get_database(&entry.database) {
                    ...
                    let metadata: Option<serde_json::Value> = entry.document_data
                        .as_ref()
                        .and_then(|d| serde_json::from_slice(d).ok());
                    
                    // Extract collection type from metadata
                    let collection_type = metadata.as_ref()
                            ...
                            if let Some(shard_config_obj) = meta.get("shardConfig") {
                                if !shard_config_obj.is_null() {
                                    if let Ok(coll) = db.get_collection(&entry.collection) {
                                        ...
                                        let num_shards = shard_config_obj.get("num_shards")
                                            .and_then(|v| v.as_u64())
                                            .unwrap_or(1) as u16;
                                        let shard_key = shard_config_obj.get("shard_key")
                                            .and_then(|v| v.as_str())
                                            .unwrap_or("_key")
                                            .to_string();
                                        let replication_factor = shard_config_obj.get("replication_factor")
                                            .and_then(|v| v.as_u64())
                                            .unwrap_or(1) as u16;
                                        ...
                                        debug!("Replicated collection {} with shard config: {:?}", entry.collection, shard_config);
                                    }
                                }
                            }
                        }
    ...
    async fn request_full_sync(&self, peer_addr: &str) -> Result<(), TransportError> {
        ...
        loop {
            ...
            match msg {
                SyncMessage::FullSyncStart { total_databases, total_documents, .. } => {
                    info!("Starting full sync: {} databases, {} documents", total_databases, total_documents);
                }
                ...
                SyncMessage::FullSyncDocuments { database, collection, data, compressed, doc_count } => {
                    ...
                    let docs: Vec<serde_json::Value> = bincode::deserialize(&docs_data).map_err(|e| {
                    ...
                    debug!("Synced {} documents to {}.{}", doc_count, database, collection);
                }
    ...
    fn collect_local_stats(&mut self) -> NodeStats {
        ...
        let cpu_usage = self.system.cpus().first().map(|c| c.cpu_usage()).unwrap_or(0.0);
    ...
    pub async fn handle_connection(
        ...
        loop {
            ...
            if tokio::io::AsyncReadExt::read_exact(&mut stream, &mut header).await.is_err() {
                break;
            }
            ...
            if tokio::io::AsyncReadExt::read_exact(&mut stream, &mut data).await.is_err() {
                break;
            }
            ...
            // Handle message
            match msg {
                SyncMessage::Heartbeat { node_id, sequence, stats } => {
                ...
                SyncMessage::IncrementalSyncRequest { from_node: _, after_sequence, max_batch_bytes } => {
                    ...
                    debug!("IncrementalSyncRequest: after_seq={}, current_seq={}, found {} entries", 
                           after_sequence, current_seq, log_entries.len());
                    
                    // Convert to SyncEntry
                    let entries: Vec<SyncEntry> = log_entries
                        .iter()
                        .map(|e| e.to_sync_entry(&hlc))
                        .collect();
                    
                    let has_more = !entries.is_empty() && entries.last().map(|e| e.sequence).unwrap_or(0) < current_seq;
                    ...
                    if let Err(e) = super::transport::ConnectionPool::write_message(&mut stream, &response).await {
                ...
                SyncMessage::FullSyncRequest { from_node } => {
                    ...
                    // Send each database
                    for db_name in &databases {
                        let db_msg = SyncMessage::FullSyncDatabase { name: db_name.clone() };
                        let _ = tokio::io::AsyncWriteExt::write_all(&mut stream, &db_msg.encode()).await;
                        ...
                        if let Ok(db) = storage.get_database(db_name) {
                            ...
                            for coll_name in colls {
                                    ...
                                    let _ = tokio::io::AsyncWriteExt::write_all(&mut stream, &coll_msg.encode()).await;
                                    ...
                                    if let Ok(coll) = db.get_collection(&coll_name) {
                                        ...
                                        for doc in coll.scan(None) {
                                            ...
                                            if batch.len() >= 1000 {
                                                ...
                                                let _ = tokio::io::AsyncWriteExt::write_all(&mut stream, &doc_msg.encode()).await;
                    ...
                    let _ = tokio::io::AsyncWriteExt::write_all(&mut stream, &complete.encode()).await;
                }
                _ => {}
            }
        }
