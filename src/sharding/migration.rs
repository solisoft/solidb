...
async fn verify_migrated_documents(
    ...
    let mut keys_by_node: std::collections::HashMap<String, Vec<(String, u16)>> = std::collections::HashMap::new();
        ...
        if let Ok(db) = storage.get_database(db_name) {
            if let Ok(physical_coll) = db.get_collection(&physical_name) {
                if physical_coll.get(&key).is_ok() {
                    ...
                    tracing::warn!("RESHARD: Local verification FAILED for {} in {}", key, physical_name);
                    // Do NOT add to verified - this doc was not migrated successfully
                }
            } else {
                tracing::debug!("RESHARD: Collection {} not found locally, trusting batch", physical_name);
                verified_keys.push(key);
            }
        }
    }
    ...
    if let Some(mgr) = cluster_manager {
        ...
        for (node_id, node_keys) in keys_by_node {
            if let Some(addr) = mgr.get_node_api_address(&node_id) {
                ...
                let mut keys_by_shard: std::collections::HashMap<u16, Vec<String>> = std::collections::HashMap::new();
                ...
                for (shard_id, shard_keys) in keys_by_shard {
                    let physical_name = format!("{}_s{}", coll_name, shard_id);
                    let url = format!("http://{}/_api/database/{}/document/{}/_verify", addr, db_name, physical_name);
                        ...
                        Ok(response) => {
                            if let Ok(body) = response.json::<serde_json::Value>().await {
                                if let Some(found) = body.get("found").and_then(|f| f.as_array()) {
                                    ...
                                    let missing = body.get("missing").and_then(|m| m.as_array()).map(|a| a.len()).unwrap_or(0);
                                    if missing > 0 {
                ...
                tracing::error!("RESHARD: Cannot find address for node {} - NOT marking docs as verified", node_id);
...
pub async fn reshard_collection<S: BatchSender>(
    ...
    current_assignments: &std::collections::HashMap<u16, crate::sharding::coordinator::ShardAssignment>,
) -> Result<(), String> {
    reshard_collection_with_journal(storage, sender, db_name, coll_name, old_shards, new_shards, my_node_id, old_assignments, current_assignments, None).await
}
...
pub async fn reshard_collection_with_journal<S: BatchSender>(
    ...
    for (shard_idx, s) in (0..scan_limit).enumerate() {
        ...
        if shard_idx > 0 && shard_idx % 10 == 0 {
            ...
            tracing::info!("RESHARD: Processed {}/{} shards in {:?}", shard_idx, scan_limit, elapsed);
        }
        ...
        let should_migrate = if s < new_shards {
            ...
            tracing::info!("RESHARD: Node {} migrating REMOVED shard {} (has local data)", my_node_id, s);
            true
        };
        ...
        let documents = physical_coll.all();
            ...
            tracing::info!("RESHARD: Scanning shard {} ({} docs) for migration...", physical_name, documents.len());
            ...
            for doc in documents {
                ...
                if s == 0 && new_shard_id != s {
                    tracing::info!("RESHARD: Document {} routes from shard {} to shard {}", id_str, s, new_shard_id);
                }
            ...
            if !docs_to_move.is_empty() {
                tracing::info!("RESHARD: Moving {} documents from shard {}", docs_to_move.len(), physical_name);
                ...
                if docs_to_move.len() > MAX_DOCS_PER_SHARD {
                    tracing::warn!("RESHARD: Limiting {} documents to {} for shard {} to prevent hanging",
                        docs_to_move.len(), MAX_DOCS_PER_SHARD, physical_name);
                ...
                for (batch_idx, batch) in docs_to_move.chunks(BATCH_SIZE).enumerate() {
                    ...
                    // Progress indicator
                    if batch_idx % 10 == 0 {
                        tracing::info!("RESHARD: Processing batch {}/{} for shard {}",
                            batch_idx + 1, (docs_to_move.len() + BATCH_SIZE - 1) / BATCH_SIZE, physical_name);
                    }
                    ...
                    let successful_keys = loop {
                        match sender.send_batch(db_name, coll_name, &config, batch_vec.clone()).await {
                            Ok(keys) => break keys,
                            Err(e) => {
                                last_error = Some(e);
                    ...
                    if successful_keys.is_empty() {
                        ...
                        if let Some(error) = last_error {
                            tracing::error!("RESHARD: Batch completely failed after retries: {}", error);
                        }
                        ...
                        if let Some(journal) = journal {
                            for (key, _) in &batch_vec {
                                let entry = MigrationJournalEntry::new(db_name, coll_name, key, s, ShardRouter::route(key, new_shards));
                        ...
                        let verified_keys = verify_migrated_documents(
                            storage, db_name, coll_name, &successful_keys, &config, current_assignments, my_node_id, None
                        ).await;
                        ...
                        let keys_to_delete = if verified_keys.len() == successful_keys.len() {
                            ...
                            let has_remote_operations = successful_keys.iter()
                                .any(|key| {
                                    let shard_id = ShardRouter::route(key, new_shards);
                                    current_assignments.get(&shard_id)
                                        .map(|a| a.primary_node != my_node_id)
                                        .unwrap_or(false)
                                });
                            ...
                            if has_remote_operations {
                                ...
                                tracing::error!("RESHARD: Local verification failed completely - skipping batch");
                            ...
                            tracing::warn!("RESHARD: Partial verification - {} verified, {} unverified",
                                verified_keys.len(), unverified_count);
                        ...
                        if let Ok(deleted) = physical_coll.delete_batch(&keys_to_delete) {
                            ...
                            if keys_to_delete.len() < batch.len() {
                                ...
                                if let Some(journal) = journal {
                                    for (key, _) in batch_vec.iter().filter(|(k, _)| !successful_keys.contains(k)) {
                                        let entry = MigrationJournalEntry::new(db_name, coll_name, key, s, ShardRouter::route(key, new_shards));
                                        journal.record_migration(entry);
                                    }
                                }
                ...
                if failure_rate > 0.1 { // More than 10% failure rate for this shard
                    ...
                    return Err(format!("RESHARD: Too many migration failures for shard {}: {}/{} failed",
                        physical_name, failed_count, docs_to_move.len()));
                }

                // Success metrics
                if moved_count > 0 {
                    tracing::info!("RESHARD: Successfully migrated {} documents from shard {} at {:.1} docs/sec",
                        moved_count, physical_name, throughput);
                }
            }
        }
    ...
    if let Ok(db) = storage.get_database(db_name) {
        ...
        tracing::info!("RESHARD: Final verification - {} total documents across {} shards", total_documents, new_shards);
        ...
        if total_documents > 0 {
            ...
            let deviation = (avg_per_shard - (total_documents as f64 / new_shards as f64).abs()).abs() / avg_per_shard;
            if deviation > 0.5 { // More than 50% deviation from average
                tracing::warn!("RESHARD: Unusual document distribution detected - deviation: {:.2}%", deviation * 100.0);
            }
        }
    }
...
impl MigrationCoordinator {
    ...
    pub async fn reshard_collection_robust<S: BatchSender>(
        ...
        for s in 0..scan_limit {
            ...
            if is_primary {
                state.shard_progress.insert(s, ShardMigrationProgress {
                    shard_id: s,
                    documents_scanned: 0,
        ...
        self.perform_scanning_phase(db_name, coll_name, old_shards, new_shards, my_node_id, old_assignments, current_assignments).await?;

        // Phase 2: Migration phase with retry logic
        self.perform_migration_phase(sender, db_name, coll_name, old_shards, new_shards, my_node_id, old_assignments, current_assignments).await?;

        // Phase 3: Verification phase
        self.perform_verification_phase(db_name, coll_name, old_shards, new_shards, my_node_id, old_assignments, current_assignments).await?;
        ...
        for (shard_id, progress) in state.shard_progress.iter() {
            if let ShardMigrationStatus::NotStarted = progress.status {
                ...
                if let Ok(physical_coll) = db.get_collection(&physical_name) {
                    ...
                    tracing::info!("MIGRATION: Scanned shard {} - {} documents", shard_id, document_count);
                }
            }
        }
    ...
    pub fn can_resume(&self) -> bool {
        let state = self.migration_state.lock().unwrap();
        matches!(state.phase, MigrationPhase::Scanning | MigrationPhase::Migrating | MigrationPhase::Verifying)
    }

    /// Record a successful migration in the journal
    pub fn record_migration(&self, entry: MigrationJournalEntry) {
        let mut journal = self.journal.lock().unwrap();
        let key = format!("{}:{}:{}", entry.db_name, entry.collection_name, entry.document_key);
...
mod tests {
    ...
    async fn test_reshard_collection_basic() {
        ...
        for i in 0..20 {
            let doc = serde_json::json!({ "_key": format!("migration_test_doc_{}", i), "value": i });
        ...
        assert!(!sent_batches.is_empty(), "Some documents should have been migrated");
    }
    ...
    async fn test_reshard_collection_no_primary() {
        ...
        assert!(result.is_ok(), "Resharding should succeed even when not primary");

        // Check that no documents were migrated (since we're not primary)
        let sent_batches = sender.sent_batches.lock().unwrap();
        assert!(sent_batches.is_empty(), "No documents should be migrated when not primary");
    }
    ...
    async fn test_reshard_collection_sender_failure() {
        ...
        assert!(result.is_ok(), "Resharding should handle sender failures gracefully");

        // Check that batches were attempted
        let sent_batches = sender.sent_batches.lock().unwrap();
        println!("Sent batches count: {}", sent_batches.len());
        assert!(!sent_batches.is_empty(), "Batches should have been attempted even with failures");
    }
    ...
    async fn test_migration_journal_idempotency() {
        ...
        // First migration run
        let result1 = coordinator.reshard_collection_robust(
            &*sender,
            "test_db",
            "test_coll",
            1,
        ...
        let result2 = coordinator.reshard_collection_robust(
            &*sender,
            "test_db",
            "test_coll",
            1,
        ...
        assert_eq!(batches1, batches2, "Idempotency failed - additional batches sent on second run");
        assert_eq!(stats1.journal_migrated, stats2.journal_migrated, "Migration counts should be identical");
    }
    ...
    async fn test_migration_failure_recovery() {
        ...
        assert!(stats.journal_migrated > 0, "Some documents should have been migrated");
        assert_eq!(stats.errors.len(), 0, "No errors should remain after successful retry");

        // Check that batches were attempted multiple times due to failures
        let batches = sender.sent_batches.lock().unwrap();
        assert!(batches.len() >= 2, "Should have attempted multiple batches due to retries");
    }
    ...
    async fn test_migration_coordinator_full_workflow() {
        ...
        let docs_to_insert = vec!["doc_0", "doc_2", "doc_3", "doc_6", "doc_9", "doc_18", "doc_19"];
        ...
        assert!(final_status.total_processed >= 7, "Should have processed at least 7 documents");
        assert!(final_status.total_migrated > 0, "Should have migrated some documents");

        // Check that we can get stats
        let stats = coordinator.get_migration_stats();
        assert!(stats.journal_migrated > 0, "Journal should record successful migrations");

        // Test cleanup
        coordinator.cleanup();
        let after_cleanup = coordinator.get_migration_stats();
        assert_eq!(after_cleanup.journal_migrated, 0, "Journal should be cleared after cleanup");
    }
    ...
    async fn test_migration_partial_failure_handling() {
        ...
        assert!(result.is_ok(), "Migration should succeed despite partial failures");

        let stats = coordinator.get_migration_stats();

        // Should have some successful migrations and some failures
        assert!(stats.journal_migrated > 0, "Should have successful migrations");
        assert!(stats.journal_failed > 0, "Should have recorded failed migrations");
        ...
        assert!(remaining_keys.contains(&"doc_3".to_string()), "Failed doc_3 should remain in source");
        assert!(remaining_keys.contains(&"doc_7".to_string()), "Failed doc_7 should remain in source");
    }
    ...
    async fn test_migration_empty_collection() {
        ...
        assert!(result.is_ok(), "Migration of empty collection should succeed");

        let stats = coordinator.get_migration_stats();
        assert_eq!(stats.total_processed, 0, "Should process 0 documents for empty collection");
        assert_eq!(stats.total_migrated, 0, "Should migrate 0 documents for empty collection");

        // No batches should be sent for empty collection
        let batches = sender.sent_batches.lock().unwrap();
        assert_eq!(batches.len(), 0, "No batches should be sent for empty collection");
    }
    ...
    async fn test_migration_multiple_shards() {
        ...
        for shard_id in 0..2 {
            current_assignments.insert(shard_id, ShardAssignment {
                shard_id,
                primary_node: "test_node".to_string(),
                replica_nodes: vec!["node2".to_string()],
            });
        ...
        let stats = coordinator.get_migration_stats();
        assert!(stats.total_processed >= 20, "Should have processed documents from both shards");
        assert!(stats.total_migrated > 0, "Should have migrated some documents to new shard locations");

        // Check progress tracking for multiple shards
        let status = coordinator.get_status();
        assert_eq!(status.shard_progress.len(), 2, "Should track progress for 2 shards");
    }
    ...
    async fn test_reshard_shrink_broadcast() {
        ...
        for shard_id in 0..4 {
            old_assignments.insert(shard_id, ShardAssignment {
                shard_id,
                primary_node: "test_node".to_string(),
                replica_nodes: vec![],
            });
        ...
        for shard_id in 0..3 {  // Only shards 0, 1, 2 will remain
            current_assignments.insert(shard_id, ShardAssignment {
                shard_id,
        ...
        let shard3_keys: Vec<String> = all_processed_keys.iter()
            .filter(|k| k.starts_with("shard3_doc_"))
            .cloned()
            .collect();

        assert!(!shard3_keys.is_empty(), "Should have processed shard 3 documents: {:?}", shard3_keys);
        assert_eq!(shard3_keys.len(), 25, "Should process all 25 documents from shard 3");
    ...
    async fn test_regression_resharding_timeout_and_limits() {
        ...
        assert!(duration.as_secs() < 10, "Resharding should complete in under 10 seconds, took {:?}", duration);

        let stats = coordinator.get_migration_stats();
        assert_eq!(stats.total_processed, 200, "Should process all 200 documents");
    }
    ...
    async fn test_regression_expansion_data_preservation() {
        ...
        current_assignments.insert(1, ShardAssignment {
            shard_id: 1,
        ...
        assert!(duration.as_secs() < 10, "Should complete within 10 seconds, took {:?}", duration);
        assert!(duration.as_secs() < 10, "Should complete within 10 seconds, took {:?}", duration);

        // Should have processed documents
        let batches = sender.sent_batches.lock().unwrap();
        assert!(!batches.is_empty(), "Should have processed documents during expansion");
    }
    ...
    async fn test_regression_circuit_breaker_prevents_hanging() {
        ...
        assert!(duration.as_millis() < 2000, "Should complete within 2 seconds even with failures, took {:?}", duration);
    ...
    impl BatchSender for PartialFailureSender {
        async fn send_batch(
            ...
            let successful_keys: Vec<String> = batch.into_iter()
                .filter_map(|(key, _)| {
