...
impl Parser {
    ...
    pub fn new(input: &str) -> SdbqlResult<Self> {
    ...
    pub(crate) fn expect(&mut self, expected: Token) -> SdbqlResult<()> {
        if self.current_token() == &expected {
            self.advance();
            Ok(())
        } else {
            Err(SdbqlError::ParseError(format!(
                "Expected {:?}, got {:?}",
                expected,
                self.current_token()
            )))
    ...
    pub fn parse(&mut self) -> SdbqlResult<Query> {
        self.parse_query(true)
    }

    /// Parse a query, optionally checking for trailing tokens (false for subqueries)
    pub(crate) fn parse_query(&mut self, check_trailing: bool) -> SdbqlResult<Query> {
...
impl Parser {
    ...
    pub(crate) fn try_parse_for_or_graph(&mut self) -> SdbqlResult<ForOrGraph> {
        ...
        let is_graph = if matches!(
            ...
            if matches!(
                ...
                matches!(self.peek_token(2), Token::Integer(_) | Token::Float(_))
                    && matches!(
                        self.peek_token(3),
                        Token::Outbound | Token::Inbound | Token::Any
                    )
...
pub fn parse(input: &str) -> SdbqlResult<Query> {
