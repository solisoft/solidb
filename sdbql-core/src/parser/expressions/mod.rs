...
impl Parser {
    ...
    pub(crate) fn parse_expression(&mut self) -> SdbqlResult<Expression> {
...
impl Parser {
    ...
    pub(super) fn parse_function_call_args(&mut self) -> SdbqlResult<Vec<Expression>> {
    ...
    pub(super) fn parse_keyword_as_function(&mut self, name: &str) -> SdbqlResult<Expression> {
    ...
    pub(super) fn parse_keyword_as_function_no_window(
        ...
        if matches!(self.current_token(), Token::LeftParen) {
            ...
            Err(SdbqlError::ParseError(error_msg.to_string()))
        }
    }
}
