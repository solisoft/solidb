//! Lexer for SDBQL query language.
//!
...
impl Lexer {
    ...
    fn read_number(&mut self) -> SdbqlResult<Token> {
        ...
        if has_dot {
            num_str
                .parse::<f64>()
                .map(Token::Float)
                .map_err(|_| SdbqlError::ParseError(format!("Invalid float number: {}", num_str)))
        } else {
            num_str
                .parse::<i64>()
                .map(Token::Integer)
                .map_err(|_| SdbqlError::ParseError(format!("Invalid integer number: {}", num_str)))
        }
    }
    ...
    fn read_string(&mut self) -> SdbqlResult<Token> {
        ...
        Err(SdbqlError::ParseError("Unterminated string".to_string()))
    }
    ...
    fn read_until_closing_brace(&mut self) -> SdbqlResult<String> {
    ...
    fn read_quoted_identifier(&mut self) -> SdbqlResult<Token> {
    ...
    fn read_bind_var(&mut self) -> SdbqlResult<Token> {
        self.advance(); // Skip '@'
    ...
    pub fn next_token(&mut self) -> SdbqlResult<Token> {
    ...
    pub fn tokenize(&mut self) -> SdbqlResult<Vec<Token>> {
