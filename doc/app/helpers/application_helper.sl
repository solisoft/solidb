...
# Origin used to build absolute URLs. Social crawlers (Facebook, LinkedIn,
# Slack, X) refuse a relative `og:image`, so the share tags have to carry a
# full origin rather than a path. Override with SITE_URL in .env when the
# site is served from somewhere other than production.
#
# A file-level `const` is not in scope once helpers are hoisted into the view
# environment, so this has to be a function.
def default_site_url() -> String
    return "https://solidb.solisoft.net"
end

# Absolute URL for a public path, e.g. site_url("/images/og-cover.png").
# getenv returns null when the variable is unset, and null carries no methods —
# coalesce with `??` before touching the value.
def site_url(path: String) -> String
    origin = getenv("SITE_URL") ?? default_site_url()
    origin = default_site_url() if origin.blank?

    if origin.ends_with("/")
        origin = origin.substring(0, len(origin) - 1)
    end

    return origin + path if path.starts_with("/")

    return origin + "/" + path
end

# True when a docs sidebar group is "active" — i.e. the current page is the
# group's own page or one of its subitems. Drives which group is expanded.
def doc_group_active(item, current)
    return true if item["slug"] == current
    ...
    found = false
    subs.each do |sub|
        found = true if sub["slug"] == current
    end
    return found
end
...
def link_to_class(text: String, url: String, css_class: String) -> String
    return "<a href=\"" + html_escape(_safe_link_url(url)) + "\" class=\"" +
        html_escape(css_class) + "\">" + html_escape(text) + "</a>"
end
