...
// [name, signature, description, category, href?] — href only when the
// function is documented on another card than #fn-<name>.
var SDBQL_FUNCS=[
...
["LEVENSHTEIN_MATCH","LEVENSHTEIN_MATCH(text, target, distance)","True when the edit distance is at most distance (0-4).","string"],
["NGRAM_SIMILARITY","NGRAM_SIMILARITY(input, target, n)","AQL n-gram similarity score (0.0 to 1.0).","string"],
["NGRAM_MATCH","NGRAM_MATCH(text, target, threshold?, ngramSize?)","True when n-gram Jaccard similarity reaches threshold (default 0.7).","string"],
["SUBSTRING_BYTES","SUBSTRING_BYTES(str, offset, len?)","SUBSTRING with UTF-8 byte offsets.","string"],
["IPV4_TO_NUMBER","IPV4_TO_NUMBER(str)","Dotted-quad IPv4 address to its 32-bit number.","string"],
...
["DATE_UTCTOLOCAL","DATE_UTCTOLOCAL(date, timezone, zoneinfo?)","UTC instant to local wall-clock time in an IANA timezone.","date"],
["DATE_LOCALTOUTC","DATE_LOCALTOUTC(date, timezone, zoneinfo?)","Local wall-clock time in an IANA timezone to the UTC instant.","date"],
["DATE_TIMEZONE","DATE_TIMEZONE()","The server's IANA timezone name.","date"],
["DATE_TIMEZONES","DATE_TIMEZONES()","Every IANA timezone name the date functions accept.","date"],
...
["TRUNC","TRUNC(num) / TRUNCATE(num)","Drops the fractional part (rounds toward zero).","numeric"],
["PRODUCT","PRODUCT(arr)","Product of the numbers in the array; nulls skipped, empty product is 1.","numeric"],
["DECAY_GAUSS","DECAY_GAUSS(value, origin, scale, offset, decay)","Gaussian decay score.","numeric"],
["DECAY_EXP","DECAY_EXP(value, origin, scale, offset, decay)","Exponential decay score.","numeric"],
["DECAY_LINEAR","DECAY_LINEAR(value, origin, scale, offset, decay)","Linear decay score.","numeric"],
["STDDEV_SAMPLE","STDDEV_SAMPLE(arr)","Sample standard deviation. Alias: STDDEV_SAMP.","numeric"],
...
["SHA1","SHA1(string)","SHA-1 hash as 40 hex chars. Broken for signatures; for compatibility and content addressing only.","crypto"],
["CRC32","CRC32(string)","CRC-32C checksum as uppercase hex. For partitioning and checksums, not security.","crypto"],
["FNV64","FNV64(string)","FNV-1a 64-bit hash as uppercase hex. For bucketing, not security.","crypto"],
...
["EXISTS","EXISTS(path, \"type\"?, typeName?)","True when the attribute is present (a stored null counts), optionally of a type.","misc"],
["IN_RANGE","IN_RANGE(value, low, high, includeLow?, includeHigh?)","True when value lies between low and high.","misc"],
["MATCHES","MATCHES(doc, example | [examples], returnIndex?)","True when the document matches an example object.","misc"],
["TRANSLATE","TRANSLATE(value, lookup, default?)","Looks value up in an object; default or value itself when absent.","misc"],
["VALUE","VALUE(doc, [path])","Nested value from an array path of names and indexes.","misc"],
["HASH","HASH(value)","Stable 52-bit hash of any value, AQL-compatible.","misc"],
["CURRENT_DATABASE","CURRENT_DATABASE()","Name of the database the query runs in.","misc"],
...
["REMOVE_VALUES","REMOVE_VALUES(arr, values)","Removes every occurrence of any of values.","array"],
["REMOVE_NTH","REMOVE_NTH(arr, position)","Removes the element at position (negative counts from the end).","array"],
["REPLACE_NTH","REPLACE_NTH(arr, position, value, padding?)","Replaces the element at position; past the end, pads and appends.","array"],
["JACCARD","JACCARD(arr1, arr2)","Jaccard index of two arrays as sets.","array"],
["INTERLEAVE","INTERLEAVE(arr1, arr2, ...)","Takes one element from each array in turn.","array"],
...
["DELTA","DELTA(series)","Consecutive differences of a numeric or {t,v} series.","date","/docs/timeseries#querying"],
["RATE","RATE(series, interval)","Change per interval (s/m/h/d) for a time series.","date","/docs/timeseries#querying"],
["FILL","FILL(series, mode|value)","Fill nulls: prev, interp, or a constant.","date","/docs/timeseries#querying"],
["RESAMPLE","RESAMPLE(series, interval)","Re-bucket a series; last value plus avg per bucket.","date","/docs/timeseries#querying"],
["APPROX_COUNT_DISTINCT","APPROX_COUNT_DISTINCT(arr)","HyperLogLog distinct count (returns a sketch with estimate).","misc","/docs/sdbql-functions-misc#fn-APPROX"],
["APPROX_PERCENTILE","APPROX_PERCENTILE(arr, p)","Approximate percentile 0–100.","misc","/docs/sdbql-functions-misc#fn-APPROX"],
["APPROX_TOP_K","APPROX_TOP_K(arr, k)","Space-Saving frequent items.","misc","/docs/sdbql-functions-misc#fn-APPROX"],
["SKETCH_MERGE","SKETCH_MERGE(s1, s2)","Merge two HyperLogLog sketches.","misc","/docs/sdbql-functions-misc#fn-APPROX"],
["MATCH_SEQ","MATCH_SEQ(events, key, steps)","Ordered event-pattern match per key.","misc"],
["SEMANTIC","SEMANTIC(doc, query, opts?)","Trigram semantic score (options.field default body).","misc","/docs/sdbql-functions-misc#fn-EMBED"],
["REDACT","REDACT(doc, keys)","Deep-remove object keys.","misc"],
["CURRENT_USER","CURRENT_USER()","Authenticated username, or null.","misc"],
["CURRENT_ROLES","CURRENT_ROLES()","Role names of the request principal.","misc","/docs/sdbql-functions-misc#fn-CURRENT_USER"],
["CAN","CAN(action [, doc])","RBAC plus document owner/_acl.","misc","/docs/sdbql-functions-misc#fn-CURRENT_USER"],
["CREATE_GRAPH","CREATE_GRAPH(name, spec)","Store a named graph in _graphs.","misc","/docs/graphs#overview"],
["DROP_GRAPH","DROP_GRAPH(name)","Remove a named graph.","misc","/docs/graphs#overview"],
["GRAPH_INFO","GRAPH_INFO(name)","Read a named graph document.","misc","/docs/graphs#overview"],
["CREATE_VIEW","CREATE_VIEW(name, {collection, fields?, analyzer?})","Create or replace a search view: a named alias that FOR can read instead of the backing collection.","misc","/docs/sdbql-functions-misc#fn-CREATE_VIEW"],
["DROP_VIEW","DROP_VIEW(name)","Remove a search view created with CREATE_VIEW.","misc","/docs/sdbql-functions-misc#fn-CREATE_VIEW"],
["SEARCH_INDEX","SEARCH_INDEX(coll, field, q [, n])","Fulltext index search.","misc","/docs/sdbql-functions-search#fulltext-functions"],
...
["EXTRACT","EXTRACT(text, schema)","LLM JSON extract matching a schema.","misc","/docs/sdbql-functions-misc#fn-EMBED"],
["CITE","CITE(answer, docs)","Lexical citations of which docs support an answer.","misc","/docs/sdbql-functions-misc#fn-EMBED"],
["GROUNDED","GROUNDED(answer, docs)","Support score for an answer against docs.","misc","/docs/sdbql-functions-misc#fn-EMBED"],
["TOKENS","TOKENS(text, analyzer?)","Tokenize with text_en or identity.","string","/docs/sdbql-functions-search#fulltext-functions"],
["PHRASE","PHRASE(text, …parts)","Consecutive token match after text_en.","string","/docs/sdbql-functions-search#fulltext-functions"],
["BOOST","BOOST(score, factor)","Scale a boolean or numeric score.","string","/docs/sdbql-functions-search#fulltext-functions"],
["SEARCH_SCORE","SEARCH_SCORE()","Score from the last SEARCH clause.","misc","/docs/sdbql-functions-search#fulltext-functions"],
["GEO_POINT","GEO_POINT(lat, lon)","GeoJSON Point.","geo"],
["GEO_POLYGON","GEO_POLYGON(rings)","GeoJSON Polygon.","geo","/docs/sdbql-functions-geo#fn-GEO_POINT"],
["GEO_LINESTRING","GEO_LINESTRING(points)","GeoJSON LineString.","geo","/docs/sdbql-functions-geo#fn-GEO_POINT"],
["GEO_CONTAINS","GEO_CONTAINS(a, b)","Polygon contains point or ring.","geo"],
["GEO_INTERSECTS","GEO_INTERSECTS(a, b)","Geometries overlap.","geo","/docs/sdbql-functions-geo#fn-GEO_CONTAINS"],
["GEO_IN_RANGE","GEO_IN_RANGE(p, origin, lo, hi)","Distance band in meters.","geo"],
["GEO_AREA","GEO_AREA(poly)","Approximate area in m².","geo","/docs/sdbql-functions-geo#fn-GEO_IN_RANGE"],
["PARSE_IDENTIFIER","PARSE_IDENTIFIER(id)","Split coll/key.","misc","/docs/sdbql-functions-misc#fn-PARSE"],
["PARSE_COLLECTION","PARSE_COLLECTION(id)","Collection part of an _id.","misc","/docs/sdbql-functions-misc#fn-PARSE"],
["PARSE_KEY","PARSE_KEY(id)","Key part of an _id.","misc","/docs/sdbql-functions-misc#fn-PARSE"],
["UNSET_RECURSIVE","UNSET_RECURSIVE(obj, keys…)","Deep-drop keys.","misc","/docs/sdbql-functions-misc#fn-RECURSIVE"],
["KEEP_RECURSIVE","KEEP_RECURSIVE(obj, keys…)","Deep-keep keys.","misc","/docs/sdbql-functions-misc#fn-RECURSIVE"],
["ZIP_OBJECT","ZIP_OBJECT(keys, values)","Build an object from two arrays.","array"],
["DATE_ROUND","DATE_ROUND(date, unit)","Alias of DATE_TRUNC.","date","/docs/sdbql-functions-date#fn-DATE_TRUNC"],
["APPLY","APPLY(name, args[])","Call a builtin by name.","misc"],
["CALL","CALL(name, …args)","Call a builtin by name.","misc","/docs/sdbql-functions-misc#fn-APPLY"],
["MINHASH","MINHASH(arr, n)","MinHash signature.","misc"],
["MINHASH_COUNT","MINHASH_COUNT(error)","Signature size for an error bound.","misc","/docs/sdbql-functions-misc#fn-MINHASH"],
["MINHASH_ERROR","MINHASH_ERROR(n)","Error for a signature size.","misc","/docs/sdbql-functions-misc#fn-MINHASH"],
];
...
var EXTRA_FUNCS = [
["UNION_DISTINCT","UNION_DISTINCT(arr1, arr2, ...)","Union of arrays without duplicates; AQL name for UNION.","array","/docs/sdbql-functions-array#fn-UNION"],
["EXP2","EXP2(x)","2 raised to the power of x.","numeric","/docs/sdbql-functions-numeric#fn-EXP"],
["VARIANCE_POPULATION","VARIANCE_POPULATION(arr)","Population variance. Alias of VARIANCE.","numeric","/docs/sdbql-functions-numeric#fn-VARIANCE"],
["NGRAM_POSITIONAL_SIMILARITY","NGRAM_POSITIONAL_SIMILARITY(input, target, n)","N-gram similarity that also credits partially matching n-grams.","string","/docs/sdbql-functions-string#fn-NGRAM_SIMILARITY"],
["IPV4_FROM_NUMBER","IPV4_FROM_NUMBER(num)","32-bit number to a dotted-quad IPv4 address.","string","/docs/sdbql-functions-string#fn-IPV4_TO_NUMBER"],
["IS_IPV4","IS_IPV4(str)","True when the string is a valid dotted-quad IPv4 address.","string","/docs/sdbql-functions-string#fn-IPV4_TO_NUMBER"],
["LEVENSHTEIN_DISTANCE","LEVENSHTEIN_DISTANCE(s1, s2)","Edit distance between strings; AQL name for LEVENSHTEIN.","string","/docs/sdbql-functions-string#fn-LEVENSHTEIN"],
["MERGE_RECURSIVE","MERGE_RECURSIVE(obj1, obj2, ...)","Deep merge objects; AQL name for DEEP_MERGE.","misc","/docs/sdbql-functions-misc#fn-DEEP_MERGE"],
["IS_DATESTRING","IS_DATESTRING(val)","True for a date string the date functions can parse. Alias of IS_DATETIME.","misc","/docs/sdbql-functions-misc#fn-IS_DATETIME"],
...
["SNAPSHOT_DIFF-doc","SNAPSHOT_DIFF(collection, t1, t2)","Diff two historical scans: inserted, updated, deleted.","document","/docs/documents#time-travel"]
  ...
  function buildFuncs() {
    ...
    funcs = SDBQL_FUNCS.map(function (f) {
      return { kind: "fn", href: f[4] || FNPAGE + f[3] + "#fn-" + f[0], label: f[0], sig: f[1],
