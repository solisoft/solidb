...
fn test_create_ttl_index() {
    ...
    let stats = col
        .create_ttl_index("idx_ttl".to_string(), "created_at".to_string(), 1)
        .unwrap();
...
fn test_ttl_expiration_manual_cleanup() {
    ...
    col.create_ttl_index("ttl".to_string(), "ts".to_string(), 2)
        .unwrap();
...
fn test_ttl_multiple_indexes() {
    ...
    col.create_ttl_index("idx1".to_string(), "short".to_string(), 1)
        .unwrap();

    // Index 2: 'long' expires in 10s
    col.create_ttl_index("idx2".to_string(), "long".to_string(), 10)
        .unwrap();
