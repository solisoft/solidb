...
fn test_filter_with_multiple_conditions() {
    ...
    products.insert(json!({"_key": "p1", "name": "Widget", "price": 10, "category": "tools"})).unwrap();
    products.insert(json!({"_key": "p2", "name": "Gadget", "price": 50, "category": "electronics"})).unwrap();
    products.insert(json!({"_key": "p3", "name": "Gizmo", "price": 30, "category": "tools"})).unwrap();
    products.insert(json!({"_key": "p4", "name": "Device", "price": 100, "category": "electronics"})).unwrap();
...
fn test_filter_with_or_condition() {
    ...
    items.insert(json!({"_key": "i1", "status": "active"})).unwrap();
    items.insert(json!({"_key": "i2", "status": "pending"})).unwrap();
    items.insert(json!({"_key": "i3", "status": "inactive"})).unwrap();
...
fn test_filter_with_not() {
    ...
    users.insert(json!({"_key": "u1", "name": "Alice", "admin": true})).unwrap();
    users.insert(json!({"_key": "u2", "name": "Bob", "admin": false})).unwrap();
    users.insert(json!({"_key": "u3", "name": "Charlie", "admin": false})).unwrap();
    
    let results = execute_query(&engine, 
        "FOR u IN users FILTER NOT u.admin RETURN u.name"
    );
...
fn test_filter_with_in_array() {
    ...
    orders.insert(json!({"_key": "o1", "status": "pending"})).unwrap();
    orders.insert(json!({"_key": "o2", "status": "shipped"})).unwrap();
    orders.insert(json!({"_key": "o3", "status": "delivered"})).unwrap();
    orders.insert(json!({"_key": "o4", "status": "cancelled"})).unwrap();
...
fn test_sort_ascending() {
    ...
    let results = execute_query(&engine, 
        "FOR n IN nums SORT n.value RETURN n.value"
    );
...
fn test_sort_descending() {
    ...
    let results = execute_query(&engine, 
        "FOR n IN nums SORT n.value DESC RETURN n.value"
    );
...
fn test_sort_multiple_fields() {
    ...
    people.insert(json!({"_key": "p1", "last_name": "Smith", "first_name": "Bob"})).unwrap();
    people.insert(json!({"_key": "p2", "last_name": "Jones", "first_name": "Alice"})).unwrap();
    people.insert(json!({"_key": "p3", "last_name": "Smith", "first_name": "Alice"})).unwrap();
...
fn test_let_binding_with_function() {
    ...
    words.insert(json!({"_key": "w1", "text": "Hello World"})).unwrap();
...
fn test_multiple_let_bindings() {
    ...
    items.insert(json!({"_key": "i1", "price": 100, "quantity": 5})).unwrap();
...
fn test_return_object_literal() {
    ...
    users.insert(json!({"_key": "u1", "name": "Alice", "age": 30})).unwrap();
...
fn test_return_computed_field() {
    ...
    products.insert(json!({"_key": "p1", "name": "Widget", "price": 10, "tax_rate": 0.1})).unwrap();
...
fn test_array_length() {
    ...
    items.insert(json!({"_key": "i1", "tags": ["a", "b", "c"]})).unwrap();
    
    let results = execute_query(&engine, 
        "FOR i IN items RETURN LENGTH(i.tags)"
    );
...
fn test_array_push() {
    ...
    let results = execute_query(&engine, 
        "FOR i IN items RETURN PUSH(i.nums, 3)"
    );
...
fn test_array_first_last() {
    ...
    items.insert(json!({"_key": "i1", "nums": [10, 20, 30]})).unwrap();
...
fn test_array_slice() {
    ...
    items.insert(json!({"_key": "i1", "nums": [1, 2, 3, 4, 5]})).unwrap();
    
    let results = execute_query(&engine, 
        "FOR i IN items RETURN SLICE(i.nums, 1, 3)"
    );
...
fn test_string_concat() {
    ...
    users.insert(json!({"_key": "u1", "first": "John", "last": "Doe"})).unwrap();
...
fn test_string_substring() {
    ...
    data.insert(json!({"_key": "d1", "text": "Hello World"})).unwrap();
    
    let results = execute_query(&engine, 
        "FOR d IN data RETURN SUBSTRING(d.text, 0, 5)"
    );
...
fn test_string_split() {
    ...
    let results = execute_query(&engine, 
        "FOR d IN data RETURN SPLIT(d.csv, ',')"
    );
...
fn test_string_trim() {
    ...
    data.insert(json!({"_key": "d1", "text": "  hello  "})).unwrap();
    
    let results = execute_query(&engine, 
        "FOR d IN data RETURN TRIM(d.text)"
    );
...
fn test_min_max() {
    ...
    nums.insert(json!({"_key": "n1", "values": [5, 2, 8, 1, 9]})).unwrap();
...
fn test_null_field_access() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("partial".to_string(), None).unwrap();
    let partial = engine.get_collection("partial").unwrap();
    
    partial.insert(json!({"_key": "p1", "name": "Alice"})).unwrap();
...
fn test_nested_field_access() {
    ...
    nested.insert(json!({"_key": "n1", "user": {"profile": {"name": "Alice"}}})).unwrap();
...
fn test_array_index_access() {
    ...
    arrays.insert(json!({"_key": "a1", "items": ["first", "second", "third"]})).unwrap();
