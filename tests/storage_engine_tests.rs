...
fn test_engine_data_dir() {
    ...
    assert!(engine.data_dir().contains(tmp_dir.path().file_name().unwrap().to_str().unwrap()));
}
...
fn test_recalculate_all_counts() {
    ...
    engine.create_collection("count_test".to_string(), None).unwrap();
...
fn test_data_persists_across_reopen() {
    ...
    // First session: create and insert
    {
        ...
        col.insert(json!({"_key": "doc1", "data": "hello"})).unwrap();
        engine.flush().unwrap();
        engine.flush_all_stats();
    }
...
fn test_count_persists_across_reopen() {
    ...
    {
        let engine = StorageEngine::new(path).unwrap();
        engine.create_collection("counted".to_string(), None).unwrap();
