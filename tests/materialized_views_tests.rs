...
fn test_create_materialized_view_basic() {
    ...
    // Create View
    let query_str = "CREATE MATERIALIZED VIEW older_users AS FOR u IN users FILTER u.age >= 30 RETURN u";
    ...
    assert_eq!(result[0], Value::String("Materialized view 'older_users' created".to_string()));
...
fn test_refresh_materialized_view() {
    ...
    assert_eq!(result[0], Value::String("Materialized view 'expensive_products' refreshed".to_string()));
