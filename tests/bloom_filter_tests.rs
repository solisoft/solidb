use serde_json::json;
use solidb::storage::columnar::{
    ColumnDef, ColumnFilter, ColumnType, ColumnarCollection, ColumnarIndexType, CompressionType,
};
...
async fn test_document_bloom_index() {
    ...
    assert!(
        col.bloom_check("username", &json!("alice").to_string()),
        "Bloom filter should contain alice"
    );
    assert!(
        col.bloom_check("username", &json!("bob").to_string()),
        "Bloom filter should contain bob"
    );
    ...
    assert!(
        col.bloom_check("username", &json!("alice").to_string()),
        "Persistence: Bloom filter should contain alice"
    );
...
async fn test_columnar_bloom_index() {
    ...
    db.create_collection("_columnar_emails".to_string(), None)
        .unwrap();
