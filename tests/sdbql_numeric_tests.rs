...
fn test_calculated_fields() {
    ...
    products
        .insert(json!({"_key": "p1", "price": 100, "quantity": 5}))
        .unwrap();
    products
        .insert(json!({"_key": "p2", "price": 50, "quantity": 10}))
        .unwrap();
...
fn test_percentage_calculation() {
    ...
    stats
        .insert(json!({"_key": "s1", "value": 25, "total": 100}))
        .unwrap();
    stats
        .insert(json!({"_key": "s2", "value": 75, "total": 300}))
        .unwrap();
    ...
    let pcts: Vec<f64> = results.iter().map(|r| r["pct"].as_f64().unwrap()).collect();
