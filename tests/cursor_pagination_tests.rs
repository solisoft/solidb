...
fn create_test_env() -> (StorageEngine, CursorStore, TempDir) {
    ...
    for i in 0..100 {
        col.insert(json!({"_key": format!("n{}", i), "value": i})).unwrap();
    }
...
async fn test_cursor_batching() {
    ...
    let (batch1, has_more1) = cursor_store.get_next_batch(&cursor_id).expect("Should find cursor");
    ...
    // Get second batch
    let (batch2, has_more2) = cursor_store.get_next_batch(&cursor_id).expect("Should find cursor");
...
async fn test_cursor_exhaustion() {
    ...
    #[allow(deprecated)] // SORT n.value implicit order might vary if not inserted sequentially, but here it is
    let query = parse("FOR n IN numbers FILTER n.value < 25 SORT n.value ASC RETURN n.value").unwrap();
    let executor = QueryExecutor::new(&engine);
    ...
    assert!(res.is_none(), "Should return None when cursor is exhausted/deleted");
}
