...
fn create_social_graph() -> (StorageEngine, TempDir) {
    ...
    people.insert(json!({"_key": "alice", "name": "Alice", "age": 30})).unwrap();
    people.insert(json!({"_key": "bob", "name": "Bob", "age": 25})).unwrap();
    people.insert(json!({"_key": "charlie", "name": "Charlie", "age": 35})).unwrap();
    people.insert(json!({"_key": "diana", "name": "Diana", "age": 28})).unwrap();
    people.insert(json!({"_key": "eve", "name": "Eve", "age": 32})).unwrap();
    
    // Create follows edge collection
    engine.create_collection("follows".to_string(), Some("edge".to_string())).unwrap();
    let follows = engine.get_collection("follows").unwrap();
    // Alice follows Bob and Charlie
    follows.insert(json!({"_from": "people/alice", "_to": "people/bob", "since": 2020})).unwrap();
    follows.insert(json!({"_from": "people/alice", "_to": "people/charlie", "since": 2021})).unwrap();
    // Bob follows Charlie and Diana
    follows.insert(json!({"_from": "people/bob", "_to": "people/charlie", "since": 2019})).unwrap();
    follows.insert(json!({"_from": "people/bob", "_to": "people/diana", "since": 2022})).unwrap();
    // Charlie follows Eve
    follows.insert(json!({"_from": "people/charlie", "_to": "people/eve", "since": 2018})).unwrap();
    // Diana follows Alice (circular)
    follows.insert(json!({"_from": "people/diana", "_to": "people/alice", "since": 2023})).unwrap();
...
fn test_multiple_vertex_collections() {
    ...
    posts.insert(json!({"_key": "post1", "title": "Hello World"})).unwrap();
    posts.insert(json!({"_key": "post2", "title": "Second Post"})).unwrap();
    
    // Create authored edge
    engine.create_collection("authored".to_string(), Some("edge".to_string())).unwrap();
    let authored = engine.get_collection("authored").unwrap();
    authored.insert(json!({"_from": "users/alice", "_to": "posts/post1"})).unwrap();
    authored.insert(json!({"_from": "users/alice", "_to": "posts/post2"})).unwrap();
