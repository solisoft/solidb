...
fn test_pipeline_chain_array() {
    let (engine, _tmp) = create_test_engine();
    let results = execute_query(&engine, "RETURN [3, 1, 2] |> SORTED() |> REVERSE() |> FIRST()");
    assert_eq!(results, vec![json!(3)]);
}
...
fn test_pipeline_filter_and_map() {
    let (engine, _tmp) = create_test_engine();
    let results =
        execute_query(&engine, "RETURN [1, 2, 3, 4, 5] |> FILTER(x -> x > 2) |> MAP(x -> x * 10)");
    // Results are floats due to multiplication
...
fn test_pipeline_in_for_loop() {
    ...
    let results = execute_query(&engine, "FOR i IN items RETURN i.values |> SORTED() |> FIRST()");
...
fn test_lambda_complex_expression() {
    let (engine, _tmp) = create_test_engine();
    let results = execute_query(&engine, "RETURN [1, 2, 3, 4] |> FILTER(x -> x > 1 AND x < 4)");
