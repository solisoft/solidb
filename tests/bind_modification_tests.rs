...
fn execute_with_binds(engine: &StorageEngine, query_str: &str, binds: HashMap<String, serde_json::Value>) -> Vec<serde_json::Value> {
...
fn test_bind_simple_string() {
    ...
    users.insert(json!({"_key": "alice", "name": "Alice"})).unwrap();
...
fn test_bind_number() {
    ...
    let results = execute_with_binds(&engine, 
        "FOR i IN items FILTER i.price >= @min_price RETURN i._key", binds);
...
fn test_bind_multiple() {
    ...
    products.insert(json!({"_key": "p1", "category": "A", "price": 100})).unwrap();
    products.insert(json!({"_key": "p2", "category": "A", "price": 200})).unwrap();
    products.insert(json!({"_key": "p3", "category": "B", "price": 150})).unwrap();
    ...
    let results = execute_with_binds(&engine, 
        "FOR p IN products FILTER p.category == @cat AND p.price <= @max RETURN p._key", binds);
...
fn test_bind_array() {
    ...
    engine.create_collection("items".to_string(), None).unwrap();
    let items = engine.get_collection("items").unwrap();
    items.insert(json!({"_key": "a", "type": "widget"})).unwrap();
    items.insert(json!({"_key": "b", "type": "gadget"})).unwrap();
    ...
    let results = execute_with_binds(&engine, 
        "FOR i IN items FILTER i.type IN @types RETURN i._key", binds);
...
fn test_bind_null() {
    ...
    let results = execute_with_binds(&engine, 
        "FOR d IN data FILTER d.value == @val RETURN d._key", binds);
...
fn test_bind_boolean() {
    ...
    let results = execute_with_binds(&engine, 
        "FOR f IN flags FILTER f.active == @is_active RETURN f._key", binds);
...
fn test_insert_with_key() {
    ...
    execute_query(&engine, "INSERT { \"_key\": 'mykey', \"name\": 'Item' } INTO docs");
...
fn test_insert_multiple_via_for() {
    ...
    engine.create_collection("numbers".to_string(), None).unwrap();
    
    execute_query(&engine, "FOR i IN 1..5 INSERT { \"value\": i } INTO numbers");
...
fn test_insert_with_bind() {
    ...
    execute_with_binds(&engine, 
        "INSERT { \"name\": @name, \"value\": @value } INTO items", binds);
...
fn test_update_single() {
    ...
    engine.create_collection("users".to_string(), None).unwrap();
    let users = engine.get_collection("users").unwrap();
    users.insert(json!({"_key": "alice", "name": "Alice", "age": 30})).unwrap();
...
fn test_update_via_for() {
    ...
    execute_query(&engine, "FOR i IN items UPDATE i WITH { \"cnt\": i.cnt + 5 } IN items");
...
fn test_update_filtered() {
    ...
    engine.create_collection("products".to_string(), None).unwrap();
    let products = engine.get_collection("products").unwrap();
    products.insert(json!({"_key": "p1", "status": "active", "views": 100})).unwrap();
    products.insert(json!({"_key": "p2", "status": "inactive", "views": 50})).unwrap();
...
fn test_update_with_bind() {
    ...
    execute_with_binds(&engine, 
        "UPDATE @key WITH { \"value\": @new_value } IN docs", binds);
...
fn test_remove_via_for() {
    ...
    execute_query(&engine, "FOR t IN temp FILTER t.keep == false REMOVE t IN temp");
...
fn test_insert_and_query() {
    ...
    // Insert
    execute_query(&engine, "INSERT { \"_key\": 'test', \"value\": 100 } INTO data");
    
    // Query
    let results = execute_query(&engine, "FOR d IN data FILTER d._key == 'test' RETURN d.value");
    assert_eq!(results.len(), 1);
...
fn test_insert_update_remove_sequence() {
    ...
    engine.create_collection("lifecycle".to_string(), None).unwrap();
    let lc = engine.get_collection("lifecycle").unwrap();
    
    // Insert
    execute_query(&engine, "INSERT { \"_key\": 'item', \"stage\": 'created' } INTO lifecycle");
    assert_eq!(lc.count(), 1);
    
    // Update
    execute_query(&engine, "UPDATE 'item' WITH { \"stage\": 'updated' } IN lifecycle");
...
fn test_partial_update() {
    ...
    // Update only field2
    execute_query(&engine, "UPDATE 'doc1' WITH { \"field2\": 'updated2' } IN docs");
...
fn test_limit_bind_variables() {
    ...
    let items = engine.get_collection("items").unwrap();
    for i in 0..10 {
        items.insert(json!({"_key": format!("i{}", i), "val": i})).unwrap();
    }
    ...
    let results = execute_with_binds(&engine, 
        "FOR i IN items SORT i.val LIMIT @offset, @count RETURN i.val", binds);
