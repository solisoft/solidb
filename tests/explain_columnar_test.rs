use solidb::storage::columnar::{ColumnarCollection, ColumnType, ColumnDef, CompressionType};
use solidb::storage::database::Database;
use solidb::storage::engine::StorageEngine;
use solidb::sdbql::{QueryExecutor, parse};
use rocksdb::DB;
...
fn test_explain_columnar_query() {
    ...
    // Manually create the column family for the columnar collection
    {
        ...
        if db_write.cf_handle(cf_name).is_none() {
             db_write.create_cf(cf_name, &rocksdb::Options::default()).unwrap();
        }
    }
    ...
    let query_str = "FOR m IN metrics COLLECT AGGREGATE avg_val = AVG(m.val) RETURN { avg: avg_val }";
    ...
    assert!(explain_result.is_ok(), "Explain failed: {:?}", explain_result.err());
