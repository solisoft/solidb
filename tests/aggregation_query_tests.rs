...
fn create_users_orders() -> (StorageEngine, TempDir) {
    ...
    users.insert(json!({"_key": "u1", "name": "Alice", "country": "USA"})).unwrap();
    users.insert(json!({"_key": "u2", "name": "Bob", "country": "UK"})).unwrap();
    users.insert(json!({"_key": "u3", "name": "Charlie", "country": "USA"})).unwrap();
    
    // Orders collection
    engine.create_collection("orders".to_string(), None).unwrap();
    let orders = engine.get_collection("orders").unwrap();
    orders.insert(json!({"_key": "o1", "user_key": "u1", "total": 100})).unwrap();
    orders.insert(json!({"_key": "o2", "user_key": "u1", "total": 200})).unwrap();
    orders.insert(json!({"_key": "o3", "user_key": "u2", "total": 150})).unwrap();
    orders.insert(json!({"_key": "o4", "user_key": "u3", "total": 300})).unwrap();
...
fn test_sort_ascending() {
    ...
    let results = execute_query(&engine, 
        "FOR s IN sales SORT s.amount ASC RETURN s.amount");
...
fn test_sort_descending() {
    ...
    let results = execute_query(&engine, 
        "FOR s IN sales SORT s.amount DESC RETURN s.amount");
...
fn test_return_calculated_field() {
    ...
    let results = execute_query(&engine, 
        "FOR s IN sales RETURN s.amount * s.quantity");
