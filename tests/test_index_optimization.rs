...
fn test_index_optimization_performance() {
    ...
    collection.create_index("idx_age".to_string(), "age".to_string(), 
        solidb::IndexType::Persistent, false).unwrap();
...
fn test_index_optimization_correctness() {
    ...
    // Insert test data
    collection.insert(json!({"name": "Alice", "age": 25})).unwrap();
    collection.insert(json!({"name": "Bob", "age": 30})).unwrap();
    collection.insert(json!({"name": "Charlie", "age": 30})).unwrap();
    collection.insert(json!({"name": "David", "age": 35})).unwrap();
