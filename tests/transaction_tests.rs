...
fn test_transaction_new() {
    ...
    assert!(
        ...
        "New transaction should have no operations"
    );
}
...
fn test_transaction_prepare() {
    ...
    assert!(!tx.is_active());
    assert!(
        tx.write_timestamp.is_some(),
        "Prepare should set write timestamp"
    );
}
...
fn create_test_tx_manager() -> (TransactionManager, TempDir) {
    ...
    let manager = TransactionManager::new(wal_path).expect("Failed to create transaction manager");
    (manager, tmp_dir)
}
