...
fn test_operator_equals() {
    ...
    assert_eq!(execute_single(&engine, "RETURN 'hello' == 'hello'"), json!(true));
}
...
fn test_operator_and() {
    ...
    assert_eq!(execute_single(&engine, "RETURN true AND false"), json!(false));
    assert_eq!(execute_single(&engine, "RETURN false AND true"), json!(false));
    assert_eq!(execute_single(&engine, "RETURN false AND false"), json!(false));
}
...
fn test_operator_or() {
    ...
    assert_eq!(execute_single(&engine, "RETURN false OR false"), json!(false));
...
fn test_operator_logical_precedence() {
    ...
    assert_eq!(execute_single(&engine, "RETURN true OR false AND false"), json!(true));
    assert_eq!(execute_single(&engine, "RETURN (true OR false) AND false"), json!(false));
}
...
fn test_operator_string_concat() {
    ...
    assert_eq!(execute_single(&engine, "RETURN CONCAT('Hello', ' ', 'World')"), json!("Hello World"));
}
...
fn test_operator_like() {
    ...
    items.insert(json!({"_key": "2", "name": "Application"})).unwrap();
    items.insert(json!({"_key": "3", "name": "Banana"})).unwrap();
    
    let results = execute_query(&engine, "FOR i IN items FILTER i.name LIKE 'App%' RETURN i.name");
...
fn test_operator_like_underscore() {
    ...
    let results = execute_query(&engine, "FOR i IN items FILTER i.name LIKE 'ca_' RETURN i.name");
...
fn test_operator_not_like() {
    ...
    let items = engine.get_collection("items").unwrap();
    items.insert(json!({"_key": "1", "name": "Apple"})).unwrap();
    items.insert(json!({"_key": "2", "name": "Banana"})).unwrap();
    items.insert(json!({"_key": "3", "name": "Cherry"})).unwrap();
    
    let results = execute_query(&engine, "FOR i IN items FILTER i.name NOT LIKE 'A%' RETURN i.name");
    assert_eq!(results.len(), 2);
}
...
fn test_operator_in_array() {
    ...
    assert_eq!(execute_single(&engine, "RETURN 2 IN [1, 2, 3]"), json!(true));
    assert_eq!(execute_single(&engine, "RETURN 5 IN [1, 2, 3]"), json!(false));
    assert_eq!(execute_single(&engine, "RETURN 'a' IN ['a', 'b', 'c']"), json!(true));
}
...
fn test_operator_not_in_array() {
    ...
    assert_eq!(execute_single(&engine, "RETURN NOT (5 IN [1, 2, 3])"), json!(true));
    assert_eq!(execute_single(&engine, "RETURN NOT (2 IN [1, 2, 3])"), json!(false));
}
...
fn test_array_access() {
    ...
    assert_eq!(execute_single(&engine, "RETURN ['a', 'b', 'c'][1]"), json!("b"));
}
...
fn test_ternary_true_condition() {
    ...
    assert_eq!(execute_single(&engine, "RETURN true ? 'yes' : 'no'"), json!("yes"));
}
...
fn test_ternary_false_condition() {
    ...
    assert_eq!(execute_single(&engine, "RETURN false ? 'yes' : 'no'"), json!("no"));
}
...
fn test_ternary_with_expression() {
    ...
    assert_eq!(execute_single(&engine, "RETURN 5 > 3 ? 'bigger' : 'smaller'"), json!("bigger"));
    assert_eq!(execute_single(&engine, "RETURN 2 > 3 ? 'bigger' : 'smaller'"), json!("smaller"));
}
...
fn test_ternary_nested() {
    ...
    // Nested ternary
    let result = execute_single(&engine, "RETURN 5 > 10 ? 'big' : (5 > 3 ? 'medium' : 'small')");
...
fn test_object_dynamic_access() {
    ...
    let result = execute_single(&engine, "LET key = 'name' RETURN {name: 'Alice'}[key]");
