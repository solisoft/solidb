...
async fn test_create_collection() {
    ...
    let (status, body) = post_json(&app, "/_api/collection", json!({"name": "users"})).await;
...
async fn test_create_duplicate_collection() {
    ...
    post_json(&app, "/_api/collection", json!({"name": "users"})).await;
    let (status, body) = post_json(&app, "/_api/collection", json!({"name": "users"})).await;
...
async fn test_list_collections() {
    ...
    post_json(&app, "/_api/collection", json!({"name": "users"})).await;
    post_json(&app, "/_api/collection", json!({"name": "products"})).await;

    let (status, body) = get(&app, "/_api/collection").await;
...
async fn test_delete_collection() {
    ...
    post_json(&app, "/_api/collection", json!({"name": "users"})).await;
    let status = delete(&app, "/_api/collection/users").await;

    assert_eq!(status, StatusCode::NO_CONTENT);

    // Verify deleted
    let (_, body) = get(&app, "/_api/collection").await;
    let collections = body["collections"].as_array().unwrap();
...
async fn test_insert_document() {
    ...
    post_json(&app, "/_api/collection", json!({"name": "users"})).await;
    let (status, body) = post_json(&app, "/_api/document/users", json!({
        "name": "Alice",
        "age": 30
    })).await;
...
async fn test_get_document() {
    ...
    post_json(&app, "/_api/collection", json!({"name": "users"})).await;
    post_json(&app, "/_api/document/users", json!({
        "_key": "alice",
        "name": "Alice"
    })).await;

    let (status, body) = get(&app, "/_api/document/users/alice").await;
...
async fn test_get_nonexistent_document() {
    ...
    post_json(&app, "/_api/collection", json!({"name": "users"})).await;
    let (status, body) = get(&app, "/_api/document/users/nonexistent").await;
...
async fn test_update_document() {
    ...
    let (status, body) = put_json(&app, "/_api/document/users/alice", json!({
        "age": 31,
        "city": "Paris"
    })).await;
...
async fn test_delete_document() {
    ...
    let status = delete(&app, "/_api/document/users/alice").await;
    assert_eq!(status, StatusCode::NO_CONTENT);

    // Verify deleted
    let (status, _) = get(&app, "/_api/document/users/alice").await;
    assert_eq!(status, StatusCode::NOT_FOUND);
}
...
async fn test_execute_query() {
    ...
    post_json(&app, "/_api/collection", json!({"name": "users"})).await;
    post_json(&app, "/_api/document/users", json!({"name": "Alice", "age": 30})).await;
    post_json(&app, "/_api/document/users", json!({"name": "Bob", "age": 25})).await;

    let (status, body) = post_json(&app, "/_api/cursor", json!({
...
async fn test_execute_query_with_sort() {
    ...
    post_json(&app, "/_api/document/users", json!({"name": "Alice", "age": 30})).await;
    post_json(&app, "/_api/document/users", json!({"name": "Bob", "age": 25})).await;
    post_json(&app, "/_api/document/users", json!({"name": "Charlie", "age": 35})).await;
...
async fn test_create_index() {
    ...
    post_json(&app, "/_api/collection", json!({"name": "users"})).await;
    let (status, body) = post_json(&app, "/_api/index/users", json!({
        "name": "idx_age",
        "field": "age",
        "type": "persistent"
    })).await;
...
async fn test_list_indexes() {
    ...
    post_json(&app, "/_api/collection", json!({"name": "users"})).await;
    post_json(&app, "/_api/index/users", json!({
        "name": "idx_age",
        "field": "age",
        "type": "persistent"
    })).await;

    let (status, body) = get(&app, "/_api/index/users").await;
...
async fn test_delete_index() {
    ...
    let status = delete(&app, "/_api/index/users/idx_age").await;
    assert_eq!(status, StatusCode::NO_CONTENT);

    // Verify deleted
    let (_, body) = get(&app, "/_api/index/users").await;
    let indexes = body["indexes"].as_array().unwrap();
...
async fn test_create_geo_index() {
    ...
    post_json(&app, "/_api/collection", json!({"name": "places"})).await;
    let (status, body) = post_json(&app, "/_api/geo/places", json!({
        "name": "idx_location",
        "field": "location"
    })).await;
...
async fn test_geo_near_query() {
    ...
    // Setup
    post_json(&app, "/_api/collection", json!({"name": "places"})).await;
    post_json(&app, "/_api/document/places", json!({
        "_key": "eiffel",
        "name": "Eiffel Tower",
    ...
    post_json(&app, "/_api/geo/places", json!({
        "name": "idx_location",
        "field": "location"
    })).await;

    // Query
    let (status, body) = post_json(&app, "/_api/geo/places/location/near", json!({
        "lat": 48.8584,
        "lon": 2.2945,
        "limit": 2
    })).await;
...
async fn test_geo_within_query() {
    ...
    let (status, body) = post_json(&app, "/_api/geo/places/location/within", json!({
        "lat": 48.8584,
        "lon": 2.2945,
        "radius": 10000
    })).await;
...
async fn test_collection_not_found_error() {
    ...
    let (status, body) = get(&app, "/_api/document/nonexistent/key").await;
...
async fn test_explain_simple_query() {
    ...
    let (status, body) = post_json(&app, "/_api/explain", json!({
...
async fn test_explain_with_let_subquery() {
    ...
    post_json(&app, "/_api/collection", json!({"name": "users"})).await;
    post_json(&app, "/_api/document/users", json!({"name": "Alice", "age": 30, "active": true})).await;
    post_json(&app, "/_api/document/users", json!({"name": "Bob", "age": 25, "active": true})).await;
    post_json(&app, "/_api/document/users", json!({"name": "Charlie", "age": 35, "active": false})).await;
