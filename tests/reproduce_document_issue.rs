...
fn test_document_cross_db_reference_failure() {
    ...
    assert_eq!(
        result[0]["value"],
        json!("v1"),
        "Global executor should find the document"
    );
    ...
    assert_eq!(
        result_db1[0]["value"],
        json!("v1"),
        "Executor in db1 context should find document via absolute ID"
    );
    ...
    assert_eq!(
        result_db2[0]["value"],
        json!("v1"),
        "Executor in db2 context should find document via absolute ID"
    );
}
