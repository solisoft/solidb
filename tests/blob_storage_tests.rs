...
fn test_put_single_blob_chunk() {
    ...
    files.insert(json!({"_key": "file1", "name": "test.txt"})).unwrap();
...
fn test_put_multiple_blob_chunks() {
    ...
    files.insert(json!({"_key": "large_file", "name": "large.bin"})).unwrap();
    ...
    // Verify all chunks
    assert_eq!(files.get_blob_chunk("large_file", 0).unwrap().unwrap(), chunk0.to_vec());
    assert_eq!(files.get_blob_chunk("large_file", 1).unwrap().unwrap(), chunk1.to_vec());
    assert_eq!(files.get_blob_chunk("large_file", 2).unwrap().unwrap(), chunk2.to_vec());
}
...
fn test_overwrite_blob_chunk() {
    ...
    assert_eq!(files.get_blob_chunk("file1", 0).unwrap().unwrap(), b"Original".to_vec());
    
    // Overwrite
    files.put_blob_chunk("file1", 0, b"Updated").unwrap();
    assert_eq!(files.get_blob_chunk("file1", 0).unwrap().unwrap(), b"Updated".to_vec());
}
...
fn test_blob_with_binary_data() {
    ...
    files.insert(json!({"_key": "binary", "name": "data.bin"})).unwrap();
...
fn test_blob_with_null_bytes() {
    ...
    files.insert(json!({"_key": "nulls", "name": "nulls.bin"})).unwrap();
...
fn test_blob_empty_chunk() {
    ...
    files.insert(json!({"_key": "empty", "name": "empty.bin"})).unwrap();
...
fn test_large_blob_chunk() {
    ...
    files.insert(json!({"_key": "large", "name": "large.bin"})).unwrap();
...
fn test_multiple_large_chunks() {
    ...
    files.insert(json!({"_key": "multipart", "name": "big.bin"})).unwrap();
...
fn test_multiple_documents_with_blobs() {
    ...
    for i in 0..5 {
        ...
        files.insert(json!({"_key": key, "name": format!("file{}.txt", i)})).unwrap();
        files.put_blob_chunk(&key, 0, format!("Content of file {}", i).as_bytes()).unwrap();
    }
...
fn test_blob_isolation_between_documents() {
    ...
    files.insert(json!({"_key": "doc1", "name": "doc1.txt"})).unwrap();
    files.insert(json!({"_key": "doc2", "name": "doc2.txt"})).unwrap();
...
fn test_blob_persistence() {
    ...
    // First session: create blob
    {
        ...
        files.insert(json!({"_key": "persistent", "name": "data.bin"})).unwrap();
        files.put_blob_chunk("persistent", 0, b"Persistent data").unwrap();
