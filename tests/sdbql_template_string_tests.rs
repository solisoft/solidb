...
fn test_template_string_simple_variable() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_var RETURN $"Hello ${doc.name}!""#,
    );
...
fn test_template_string_multiple_expressions() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_multi RETURN $"${doc.first} ${doc.last}""#,
    );
...
fn test_template_string_with_arithmetic() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_arith RETURN $"Total: ${doc.price * doc.qty}""#,
    );
...
fn test_template_string_with_function_call() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_func RETURN $"Name: ${UPPER(doc.name)}""#,
    );
...
fn test_template_string_nested_field_access() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_nested RETURN $"City: ${doc.address.city}""#,
    );
...
fn test_template_string_number_coercion() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_num RETURN $"Count: ${doc.count}""#,
    );
...
fn test_template_string_float_coercion() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_float RETURN $"Price: $$${doc.price}""#,
    );
...
fn test_template_string_boolean_coercion() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_bool RETURN $"Active: ${doc.active}""#,
    );
...
fn test_template_string_null_coercion() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_null RETURN $"Value: ${doc.value}""#,
    );
...
fn test_template_string_with_optional_chaining() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_opt RETURN $"City: ${doc.user?.address?.city}""#,
    );
...
fn test_template_string_with_null_coalescing() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_coal RETURN $"Hello ${doc.nickname ?? doc.name}!""#,
    );
...
fn test_template_string_ternary_expression() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_tern RETURN $"Status: ${doc.age >= 18 ? 'Adult' : 'Minor'}""#,
    );
...
fn test_template_string_with_concat() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_concat RETURN $"${CONCAT(doc.a, ' ', doc.b)}!""#,
    );
    assert_eq!(result, vec![json!("Hello World!")]);
}
...
fn test_template_string_only_expression() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_only_expr RETURN $"${doc.value}""#,
    );
...
fn test_template_string_in_object() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_in_obj RETURN {greeting: $"Hello ${doc.name}!"}"#,
    );
...
fn test_template_string_in_array() {
    ...
    let result = execute_query(
        &engine,
        r#"FOR doc IN tpl_in_arr RETURN [$"Hi ${doc.name}", $"Bye ${doc.name}"]"#,
    );
