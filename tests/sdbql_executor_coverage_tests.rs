...
use solidb::{parse, QueryExecutor, BindVars};
...
fn execute_query(engine: &StorageEngine, query_str: &str) -> Vec<serde_json::Value> {
    ...
    executor.execute(&query).expect(&format!("Query failed: {}", query_str))
}

/// Execute a query with bind variables
fn execute_with_binds(engine: &StorageEngine, query_str: &str, binds: BindVars) -> Vec<serde_json::Value> {
    let query = parse(query_str).expect(&format!("Failed to parse: {}", query_str));
...
fn execute_with_database(engine: &StorageEngine, db_name: &str, query_str: &str) -> Vec<serde_json::Value> {
    let query = parse(query_str).expect(&format!("Failed to parse: {}", query_str));
...
fn execute_with_db_and_binds(engine: &StorageEngine, db_name: &str, query_str: &str, binds: BindVars) -> Vec<serde_json::Value> {
    let query = parse(query_str).expect(&format!("Failed to parse: {}", query_str));
...
fn explain_query(engine: &StorageEngine, query_str: &str) -> solidb::sdbql::QueryExplain {
    let query = parse(query_str).expect(&format!("Failed to parse: {}", query_str));
    let executor = QueryExecutor::new(engine);
    executor.explain(&query).expect(&format!("Explain failed: {}", query_str))
}
...
fn create_seeded_engine() -> (StorageEngine, TempDir) {
    ...
    users.insert(json!({"_key": "alice", "name": "Alice", "age": 30, "dept": "eng"})).unwrap();
    users.insert(json!({"_key": "bob", "name": "Bob", "age": 25, "dept": "eng"})).unwrap();
    users.insert(json!({"_key": "charlie", "name": "Charlie", "age": 35, "dept": "sales"})).unwrap();
    users.insert(json!({"_key": "diana", "name": "Diana", "age": 28, "dept": "sales"})).unwrap();
    users.insert(json!({"_key": "eve", "name": "Eve", "age": 32, "dept": "eng"})).unwrap();
...
fn test_bind_var_string() {
    ...
    let results = execute_with_binds(&engine, 
        "FOR u IN users FILTER u.name == @name RETURN u._key", binds);
...
fn test_bind_var_number() {
    ...
    let results = execute_with_binds(&engine, 
        "FOR u IN users FILTER u.age >= @minAge RETURN u.name", binds);
...
fn test_bind_var_array() {
    ...
    let results = execute_with_binds(&engine, 
        "FOR u IN users FILTER u.dept IN @depts RETURN u.name", binds);
...
fn test_bind_var_in_limit() {
    ...
    let results = execute_with_binds(&engine, 
        "FOR u IN users LIMIT @limit RETURN u.name", binds);
...
fn test_executor_with_database() {
    ...
    items.insert(json!({"_key": "i1", "name": "Item 1"})).unwrap();
    
    let results = execute_with_database(&engine, "testdb", 
...
fn test_executor_with_database_and_binds() {
    ...
    db.create_collection("items".to_string(), None).unwrap();
    let items = db.get_collection("items").unwrap();
    items.insert(json!({"_key": "i1", "name": "Item 1", "price": 100})).unwrap();
    items.insert(json!({"_key": "i2", "name": "Item 2", "price": 200})).unwrap();
    
    let mut binds = HashMap::new();
    binds.insert("minPrice".to_string(), json!(150));
    
    let results = execute_with_db_and_binds(&engine, "testdb", 
        "FOR i IN items FILTER i.price >= @minPrice RETURN i.name", binds);
    assert_eq!(results.len(), 1);
...
fn test_collect_basic() {
    ...
    let results = execute_query(&engine, 
        "FOR u IN users COLLECT dept = u.dept RETURN dept");
    assert_eq!(results.len(), 2); // eng, sales
}
...
fn test_remove_operation() {
    ...
    items.insert(json!({"_key": "item1", "name": "Test"})).unwrap();
    items.insert(json!({"_key": "item2", "name": "Other"})).unwrap();
...
fn test_sort_three_fields() {
    ...
    data.insert(json!({"_key": "1", "a": 1, "b": 1, "c": 1})).unwrap();
    data.insert(json!({"_key": "2", "a": 1, "b": 1, "c": 2})).unwrap();
    data.insert(json!({"_key": "3", "a": 1, "b": 2, "c": 1})).unwrap();
    data.insert(json!({"_key": "4", "a": 2, "b": 1, "c": 1})).unwrap();
...
fn test_graph_inbound_traversal() {
    ...
    engine.create_collection("links".to_string(), Some("edge".to_string())).unwrap();
    let links = engine.get_collection("links").unwrap();
    links.insert(json!({"_from": "nodes/a", "_to": "nodes/b"})).unwrap();
    links.insert(json!({"_from": "nodes/c", "_to": "nodes/b"})).unwrap();
...
fn test_graph_any_direction() {
    ...
    let links = engine.get_collection("links").unwrap();
    links.insert(json!({"_from": "nodes/a", "_to": "nodes/b"})).unwrap();
    links.insert(json!({"_from": "nodes/b", "_to": "nodes/c"})).unwrap();
    
    // B connected to both A (inbound) and C (outbound)
    let results = execute_query(&engine, 
        "FOR v IN 1..1 ANY 'nodes/b' links RETURN v.name");
    assert_eq!(results.len(), 2);
}
...
fn test_explain_with_let() {
    ...
    let explain = explain_query(&engine, "LET x = 10 FOR u IN users FILTER u.age > x RETURN u.name");
...
fn test_regex_match_operator() {
    ...
    engine.create_collection("emails".to_string(), None).unwrap();
    let emails = engine.get_collection("emails").unwrap();
    emails.insert(json!({"_key": "1", "email": "alice@example.com"})).unwrap();
    emails.insert(json!({"_key": "2", "email": "bob@test.org"})).unwrap();
    emails.insert(json!({"_key": "3", "email": "charlie@example.com"})).unwrap();
...
fn test_regex_not_match_operator() {
    ...
    engine.create_collection("items".to_string(), None).unwrap();
    let items = engine.get_collection("items").unwrap();
    items.insert(json!({"_key": "1", "name": "Test123"})).unwrap();
    items.insert(json!({"_key": "2", "name": "NoNumbers"})).unwrap();
    items.insert(json!({"_key": "3", "name": "Has456"})).unwrap();
...
fn test_sha256_function() {
    ...
    assert_eq!(results[0], json!("2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824"));
}
...
fn test_filter_no_match() {
    ...
    let results = execute_query(&engine, 
        "FOR u IN users FILTER u.age > 100 RETURN u");
    assert_eq!(results.len(), 0);
}
...
fn test_deeply_nested_access() {
    ...
    let results = execute_query(&engine, 
        "FOR d IN deep RETURN d.level1.level2.level3.value");
...
fn test_null_field_access() {
    ...
    let results = execute_query(&engine, 
        "FOR d IN data RETURN d.nonexistent");
...
fn test_complex_query_all_clauses() {
    ...
    let results = execute_with_binds(&engine, r#"
...
fn test_nested_for_loops() {
    ...
    engine.create_collection("categories".to_string(), None).unwrap();
    let categories = engine.get_collection("categories").unwrap();
    categories.insert(json!({"_key": "c1", "name": "Cat1"})).unwrap();
    categories.insert(json!({"_key": "c2", "name": "Cat2"})).unwrap();
    
    engine.create_collection("products".to_string(), None).unwrap();
    let products = engine.get_collection("products").unwrap();
    products.insert(json!({"_key": "p1", "name": "Prod1", "category": "c1"})).unwrap();
    products.insert(json!({"_key": "p2", "name": "Prod2", "category": "c1"})).unwrap();
    products.insert(json!({"_key": "p3", "name": "Prod3", "category": "c2"})).unwrap();
    
    let results = execute_query(&engine, r#"
        FOR c IN categories
            FOR p IN products
...
fn test_special_characters_in_strings() {
    ...
    engine.create_collection("data".to_string(), None).unwrap();
    let data = engine.get_collection("data").unwrap();
    data.insert(json!({"_key": "1", "name": "O'Brien", "quote": "He said \"Hello\""})).unwrap();
...
fn test_unicode_data() {
    ...
    engine.create_collection("data".to_string(), None).unwrap();
    let data = engine.get_collection("data").unwrap();
    data.insert(json!({"_key": "1", "jp": "日本語", "emoji": "🎉"})).unwrap();
