...
fn test_create_index() {
    ...
    engine.create_collection("indexed".to_string(), None).unwrap();
...
fn test_create_unique_index() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("unique_indexed".to_string(), None).unwrap();
...
fn test_list_indexes() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("multi_indexed".to_string(), None).unwrap();
...
fn test_drop_index() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("drop_indexed".to_string(), None).unwrap();
...
fn test_index_lookup() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("lookup_test".to_string(), None).unwrap();
    ...
    // Insert documents
    collection.insert(json!({"_key": "doc1", "status": "active", "name": "Item 1"})).unwrap();
    collection.insert(json!({"_key": "doc2", "status": "inactive", "name": "Item 2"})).unwrap();
    collection.insert(json!({"_key": "doc3", "status": "active", "name": "Item 3"})).unwrap();
...
fn test_compound_index() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("compound".to_string(), None).unwrap();
...
fn test_rebuild_indexes() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("rebuild_test".to_string(), None).unwrap();
    let collection = engine.get_collection("rebuild_test").unwrap();
    
    // Insert documents without indexing
    for i in 0..10 {
        collection.insert_no_index(json!({"name": format!("item{}", i), "value": i})).unwrap();
    }
    ...
    // Rebuild indexes
    let count = collection.rebuild_all_indexes().expect("Rebuild should succeed");
    assert!(count > 0);
}
...
fn test_scan_with_limit() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("scan_test".to_string(), None).unwrap();
...
fn test_scan_all() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("scan_all".to_string(), None).unwrap();
...
fn test_get_many() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("getmany".to_string(), None).unwrap();
...
fn test_get_many_with_missing() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("getmany_missing".to_string(), None).unwrap();
    let collection = engine.get_collection("getmany_missing").unwrap();
    
    collection.insert(json!({"_key": "exists", "value": 1})).unwrap();
...
fn test_collection_stats() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("stats_test".to_string(), None).unwrap();
    let collection = engine.get_collection("stats_test").unwrap();
    
    // Insert documents
    for i in 0..20 {
        collection.insert(json!({"data": format!("item_{}", i)})).unwrap();
    }
...
fn test_recount_documents() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("recount_test".to_string(), None).unwrap();
...
fn test_disk_usage() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("disk_test".to_string(), None).unwrap();
    let collection = engine.get_collection("disk_test").unwrap();
    
    // Insert some data
    for i in 0..10 {
        collection.insert(json!({"data": "x".repeat(1000), "index": i})).unwrap();
    }
    
    let usage = collection.disk_usage();
    // DiskUsage has sst_files_size, live_data_size, etc.
    assert!(usage.sst_files_size >= 0 || usage.live_data_size >= 0);
}
...
fn test_truncate_collection() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("truncate_test".to_string(), None).unwrap();
...
fn test_truncate_empty_collection() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("truncate_empty".to_string(), None).unwrap();
    let collection = engine.get_collection("truncate_empty").unwrap();
    
    let deleted = collection.truncate().expect("Truncate empty should succeed");
    assert_eq!(deleted, 0);
}
...
fn test_batch_delete() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("batch_del".to_string(), None).unwrap();
    let collection = engine.get_collection("batch_del").unwrap();
    
    // Insert documents
    for i in 0..10 {
        collection.insert(json!({"_key": format!("doc{}", i), "value": i})).unwrap();
    }
    
    // Delete half
    let keys: Vec<String> = (0..5).map(|i| format!("doc{}", i)).collect();
    let deleted = collection.delete_batch(&keys).expect("Batch delete should succeed");
...
fn test_update_with_correct_revision() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("rev_test".to_string(), None).unwrap();
    let collection = engine.get_collection("rev_test").unwrap();
    
    // Insert document
    let doc = collection.insert(json!({"_key": "doc1", "value": 1})).unwrap();
...
fn test_update_with_wrong_revision() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("rev_wrong".to_string(), None).unwrap();
    let collection = engine.get_collection("rev_wrong").unwrap();
    
    // Insert document
    collection.insert(json!({"_key": "doc1", "value": 1})).unwrap();
...
fn test_edge_collection_type() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("typed_edges".to_string(), Some("edge".to_string())).unwrap();
...
fn test_blob_put_and_get() {
    ...
    collection.put_blob_chunk("myblob", 0, data).expect("Put blob should succeed");
    
    // Retrieve
    let retrieved = collection.get_blob_chunk("myblob", 0)
...
fn test_blob_multiple_chunks() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("multi_blobs".to_string(), None).unwrap();
    let collection = engine.get_collection("multi_blobs").unwrap();
    
    // Store multiple chunks
    for i in 0..5 {
        let data = format!("Chunk {} data", i);
        collection.put_blob_chunk("bigfile", i, data.as_bytes()).unwrap();
    }
...
fn test_blob_delete() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("del_blobs".to_string(), None).unwrap();
    let collection = engine.get_collection("del_blobs").unwrap();
    
    // Store blob
    collection.put_blob_chunk("todelete", 0, b"data").unwrap();
    collection.put_blob_chunk("todelete", 1, b"more data").unwrap();
    
    // Delete all blob data
    collection.delete_blob_data("todelete").expect("Delete blob should succeed");
...
fn test_upsert_batch_insert() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("upsert_ins".to_string(), None).unwrap();
    ...
    let count = collection.upsert_batch(docs).expect("Upsert should succeed");
    assert_eq!(count, 10);
...
fn test_upsert_batch_update() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("upsert_upd".to_string(), None).unwrap();
...
fn test_special_characters_in_key() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("special".to_string(), None).unwrap();
    ...
    for key in keys {
        collection.insert(json!({"_key": key, "data": "test"})).unwrap();
...
fn test_unicode_in_document() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("unicode".to_string(), None).unwrap();
...
fn test_deeply_nested_document() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("nested".to_string(), None).unwrap();
...
fn test_array_in_document() {
    let (engine, _tmp) = create_test_engine();
    engine.create_collection("arrays".to_string(), None).unwrap();
