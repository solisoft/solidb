...
impl TestContext {
    async fn new() -> Self {
        ...
        let app = create_router(engine, None, None, None, None, script_stats, None, None, 0);
        ...
        if !db_status.is_success() {
            let body = axum::body::to_bytes(db_response.into_body(), 1024 * 1024)
                .await
                .unwrap();
            let json: Value = serde_json::from_slice(&body).unwrap_or(json!({}));
...
async fn test_submit_contribution_feature() {
    ...
    let (status, json) = ctx
        .post(
            "/_api/database/testdb/ai/contributions",
            json!({
                "type": "feature",
    ...
    assert!(
        json["id"].is_string(),
        "Response should contain contribution id"
    );
    assert_eq!(
        json["status"], "success",
...
async fn test_list_contributions_with_limit() {
    ...
    let (status, json) = ctx
        .get("/_api/database/testdb/ai/contributions?limit=2")
        .await;
...
async fn test_get_contribution_not_found() {
    ...
    let (status, _json) = ctx
        .get("/_api/database/testdb/ai/contributions/nonexistent-id")
        .await;
...
async fn test_claim_task() {
    ...
    let (_, tasks_json) = ctx
        .get("/_api/database/testdb/ai/tasks?status=pending")
        .await;
...
async fn test_complete_task_creates_next_task() {
    ...
    assert!(pending_tasks
        .iter()
        .any(|t| t["task_type"] == "generate_code"));
}
...
async fn test_register_agent() {
    ...
    let (status, json) = ctx
        .post(
            "/_api/database/testdb/ai/agents",
            json!({
                "name": "test-analyzer",
                "agent_type": "analyzer",
...
async fn test_reject_contribution() {
    ...
    let (status, json) = ctx
        .post(
            &format!(
                "/_api/database/testdb/ai/contributions/{}/reject",
                contrib_id
            ),
            json!({
...
async fn test_full_pipeline_low_risk() {
    ...
    let task_id = get_pending_task(&ctx, "analyze_contribution")
        .await
        .unwrap();
    process_task(
        &ctx,
