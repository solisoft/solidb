...
fn create_seeded_engine() -> (StorageEngine, TempDir) {
    ...
    users.insert(json!({"_key": "alice", "name": "Alice", "age": 30, "city": "Paris", "active": true})).unwrap();
    users.insert(json!({"_key": "bob", "name": "Bob", "age": 25, "city": "London", "active": true})).unwrap();
    users.insert(json!({"_key": "charlie", "name": "Charlie", "age": 35, "city": "Paris", "active": false})).unwrap();
    users.insert(json!({"_key": "diana", "name": "Diana", "age": 28, "city": "Berlin", "active": true})).unwrap();
    users.insert(json!({"_key": "eve", "name": "Eve", "age": 32, "city": "London", "active": true})).unwrap();
    
    // Create products collection
    engine.create_collection("products".to_string(), None).unwrap();
    let products = engine.get_collection("products").unwrap();
    products.insert(json!({"_key": "p1", "name": "Laptop", "price": 999.99, "category": "electronics"})).unwrap();
    products.insert(json!({"_key": "p2", "name": "Phone", "price": 599.99, "category": "electronics"})).unwrap();
    products.insert(json!({"_key": "p3", "name": "Book", "price": 19.99, "category": "books"})).unwrap();
    products.insert(json!({"_key": "p4", "name": "Headphones", "price": 149.99, "category": "electronics"})).unwrap();
...
fn test_for_return_object() {
    ...
    let results = execute_query(&engine, "FOR doc IN users RETURN { name: doc.name, age: doc.age }");
    assert_eq!(results.len(), 5);
    
    let first = results.iter().find(|v| v.get("name") == Some(&json!("Alice"))).unwrap();
...
fn test_filter_equals() {
    ...
    let results = execute_query(&engine, "FOR doc IN users FILTER doc.city == 'Paris' RETURN doc.name");
...
fn test_filter_greater_than() {
    ...
    let results = execute_query(&engine, "FOR doc IN users FILTER doc.age > 30 RETURN doc.name");
...
fn test_filter_less_than_or_equal() {
    ...
    let results = execute_query(&engine, "FOR doc IN users FILTER doc.age <= 28 RETURN doc.name");
...
fn test_filter_boolean() {
    ...
    let results = execute_query(&engine, "FOR doc IN users FILTER doc.active == true RETURN doc.name");
...
fn test_filter_like() {
    ...
    // LIKE with % wildcard
    let results = execute_query(&engine, "FOR doc IN users FILTER doc.name LIKE 'A%' RETURN doc.name");
    assert_eq!(results.len(), 1);
    assert_eq!(results[0], json!("Alice"));
}
...
fn test_sort_descending() {
    ...
    let results = execute_query(&engine, "FOR doc IN users SORT doc.age DESC RETURN doc.name");
    assert_eq!(results.len(), 5);
...
fn test_limit_offset_count() {
    ...
    let results = execute_query(&engine, "FOR doc IN users SORT doc.age ASC LIMIT 1, 2 RETURN doc.name");
    assert_eq!(results.len(), 2);
...
fn test_sort_and_limit() {
    ...
    // Top 3 oldest users
    let results = execute_query(&engine, "FOR doc IN users SORT doc.age DESC LIMIT 3 RETURN doc.name");
    assert_eq!(results.len(), 3);
...
fn test_insert_new_document() {
    ...
    // Verify insertion
    let results = execute_query(&engine, "FOR doc IN users FILTER doc._key == 'frank' RETURN doc");
    assert_eq!(results.len(), 1);
...
fn test_insert_with_return() {
    ...
    let results = execute_query(&engine, "FOR doc IN users FILTER doc._key == 'grace' RETURN doc");
    assert_eq!(results.len(), 1);
...
fn test_update_document() {
    ...
    // Verify update
    let results = execute_query(&engine, "FOR doc IN users FILTER doc._key == 'alice' RETURN doc.age");
...
fn test_nested_field_access() {
    ...
    engine.create_collection("profiles".to_string(), None).unwrap();
    let profiles = engine.get_collection("profiles").unwrap();
    profiles.insert(json!({
        "_key": "p1",
        "user": {
            "name": "Alice",
            "address": {
    ...
    let results = execute_query(&engine, 
        "FOR p IN profiles RETURN p.user.address.city");
...
fn test_array_in_document() {
    ...
    engine.create_collection("orders".to_string(), None).unwrap();
...
fn test_create_and_use_index() {
    ...
    let users = engine.get_collection("users").unwrap();
    users.create_index("city_idx".to_string(), vec!["city".to_string()], IndexType::Persistent, false).unwrap();
...
fn test_unique_index_creation() {
    ...
    // Create unique index
    let result = emails.create_index("email_unique".to_string(), vec!["email".to_string()], IndexType::Hash, true);
    assert!(result.is_ok(), "Should create unique index: {:?}", result.err());
    
    // Insert first document
    emails.insert(json!({"_key": "u1", "email": "test@example.com"})).unwrap();
...
fn test_edge_collection() {
    ...
    engine.create_collection("people".to_string(), None).unwrap();
    let people = engine.get_collection("people").unwrap();
    people.insert(json!({"_key": "alice", "name": "Alice"})).unwrap();
    people.insert(json!({"_key": "bob", "name": "Bob"})).unwrap();
    people.insert(json!({"_key": "charlie", "name": "Charlie"})).unwrap();
    
    // Create edge collection
    engine.create_collection("knows".to_string(), Some("edge".to_string())).unwrap();
    let knows = engine.get_collection("knows").unwrap();
    knows.insert(json!({"_from": "people/alice", "_to": "people/bob", "since": 2020})).unwrap();
    knows.insert(json!({"_from": "people/bob", "_to": "people/charlie", "since": 2021})).unwrap();
...
fn test_graph_outbound_traversal() {
    ...
    // Setup graph
    engine.create_collection("vertices".to_string(), None).unwrap();
    ...
    edges.insert(json!({"_from": "vertices/a", "_to": "vertices/b"})).unwrap();
    edges.insert(json!({"_from": "vertices/a", "_to": "vertices/c"})).unwrap();
