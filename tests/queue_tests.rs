...
use solidb::queue::{QueueWorker, Job, JobStatus};
...
fn create_test_env() -> (Arc<StorageEngine>, Arc<QueueWorker>, TempDir) {
    ...
    db.create_collection("_cron_jobs".to_string(), None).unwrap();
...
async fn test_job_execution_success() {
    ...
    for i in 0..50 { // Wait up to 5 seconds
        ...
        if i % 10 == 0 {
             println!("Waiting... status={:?}, retry={}", updated_job.status, updated_job.retry_count);
...
async fn test_cron_scheduling() {
    ...
    cron_jobs.insert(serde_json::to_value(&cron_job).unwrap()).unwrap();
    ...
    let updated_cron: solidb::queue::CronJob = serde_json::from_value(updated_cron_doc.to_value()).unwrap();
