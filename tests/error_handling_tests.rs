...
fn test_error_invalid_operator() {
    let result = parse("FOR doc IN users FILTER doc.name === 'test' RETURN doc");
...
fn test_query_without_return() {
    ...
    let query = parse("FOR doc IN users FILTER doc.age > 30");
...
fn test_error_query_collection_not_found() {
    ...
    assert!(result.is_err(), "Query on nonexistent collection should fail");
}
...
fn test_error_geo_index_duplicate() {
    ...
    places.create_geo_index("location_idx".to_string(), "location".to_string()).unwrap();
...
fn test_query_with_null_values() {
    ...
    users.insert(json!({"_key": "u1", "name": "Alice", "email": null})).unwrap();
...
fn test_insert_deeply_nested_document() {
    ...
    let query = parse("FOR doc IN nested RETURN doc.level1.level2.level3.level4.level5.value").unwrap();
    let executor = QueryExecutor::new(&engine);
