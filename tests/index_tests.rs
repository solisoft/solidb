...
fn test_hash_index_create() {
    ...
    assert!(result.is_ok(), "Should create hash index: {:?}", result.err());
}
...
fn test_hash_index_list() {
    ...
    users.create_index("idx1".to_string(), vec!["field1".to_string()], IndexType::Hash, false).unwrap();
    users.create_index("idx2".to_string(), vec!["field2".to_string()], IndexType::Hash, false).unwrap();
...
fn test_persistent_index_with_data() {
    ...
    // Insert data first
    products.insert(json!({"_key": "p1", "name": "Laptop", "price": 999})).unwrap();
    products.insert(json!({"_key": "p2", "name": "Phone", "price": 599})).unwrap();
    products.insert(json!({"_key": "p3", "name": "Tablet", "price": 399})).unwrap();
...
fn test_compound_index_with_data() {
    ...
    users.insert(json!({"_key": "u1", "city": "Paris", "age": 30})).unwrap();
    users.insert(json!({"_key": "u2", "city": "London", "age": 25})).unwrap();
    users.insert(json!({"_key": "u3", "city": "Paris", "age": 35})).unwrap();
...
fn test_fulltext_index_create() {
    ...
    engine.create_collection("articles".to_string(), None).unwrap();
...
fn test_geo_index_create() {
    ...
    engine.create_collection("places".to_string(), None).unwrap();
...
fn test_geo_near_search() {
    ...
    places.create_geo_index("location_geo".to_string(), "location".to_string()).unwrap();
...
fn test_geo_list_indexes() {
    ...
    places.create_geo_index("geo1".to_string(), "location1".to_string()).unwrap();
    places.create_geo_index("geo2".to_string(), "location2".to_string()).unwrap();
...
fn test_rebuild_all_indexes() {
    ...
    for i in 0..50 {
        col.insert(json!({"field": format!("value_{}", i)})).unwrap();
    }
...
fn test_duplicate_index_name_error() {
    ...
    engine.create_collection("dup_test".to_string(), None).unwrap();
    let col = engine.get_collection("dup_test").unwrap();
    
    // Create first index
    col.create_index("my_idx".to_string(), vec!["field1".to_string()], IndexType::Hash, false).unwrap();
    
    // Try to create another with same name - should fail
    let result = col.create_index("my_idx".to_string(), vec!["field2".to_string()], IndexType::Hash, false);
