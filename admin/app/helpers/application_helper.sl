...
def capitalize(text: String) -> String
    ...
    return text.substring(0, 1).upcase() + text.substring(1, text.length())
end
...
def link_to_class(text: String, url: String, css_class: String) -> String
    opening = "<a href=\"" + html_escape(_safe_link_url(url)) + "\" class=\"" + html_escape(css_class) + "\">"
    return opening + html_escape(text) + "</a>"
end
