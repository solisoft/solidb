var talksMessages = {
  ...
  exports: {
    ...
    getParticipantAvatarColor(participant, index) {
      const colors = ['bg-gradient-to-br from-indigo-500 to-purple-600', 'bg-gradient-to-br from-green-500 to-teal-600', 'bg-gradient-to-br from-orange-500 to-red-600', 'bg-gradient-to-br from-blue-500 to-cyan-600', 'bg-gradient-to-br from-pink-500 to-rose-600'];
      return colors[index % colors.length];
    }
  },
  template: (template, expressionTypes, bindingTypes, getComponent) => template('<div class="flex-1 relative min-h-0 flex flex-col"><div expr16="expr16" ref="messagesArea" class="flex-1 overflow-y-auto px-6 pb-6 pt-20 space-y-1"><div expr17="expr17" class="text-center text-gray-500 py-8"></div><template expr18="expr18"></template></div><div expr68="expr68" class="absolute bottom-6 right-8 z-10 animate-fade-in"></div></div>', [{
    ...
    template: template('<div class="relative flex items-center py-4"><div class="flex-grow border-t border-gray-800"></div><span expr19="expr19" class="flex-shrink mx-4 text-xs font-bold text-gray-500 bg-[#1A1D21] px-2 uppercase tracking-wider"> </span><div class="flex-grow border-t border-gray-800"></div></div><div expr20="expr20"></div>', [{
      ...
      getKey: null,
      condition: null,
      template: template('<div expr21="expr21"> </div><div class="flex-1 min-w-0"><div class="flex items-baseline mb-1"><span expr22="expr22" class="font-bold text-white mr-2 hover:underline cursor-pointer"> </span><span expr23="expr23" class="text-xs text-gray-500"> </span></div><div expr24="expr24"><span expr25="expr25"></span></div><div expr38="expr38" class="mt-3"></div><div expr47="expr47" class="mt-3 rounded-lg overflow-hidden border border-gray-700 bg-[#121016] shadow-inner"></div><div expr51="expr51" class="mt-2 flex flex-wrap\n                            gap-2"></div><div class="mt-0.5 flex flex-wrap gap-1.5 items-center"><div expr59="expr59" class="relative group/reaction"></div><div class="relative group/emoji"><button expr62="expr62" class="p-1.5 rounded text-gray-500 hover:text-white hover:bg-gray-700\n                                    transition-colors opacity-0 group-hover:opacity-100"><i class="far fa-smile text-sm"></i></button></div><div class="relative group/reply"><button expr63="expr63" class="p-1.5 rounded text-gray-500 hover:text-white hover:bg-gray-700\n                                    transition-colors opacity-0 group-hover:opacity-100" title="Reply in thread"><i class="fas fa-reply text-sm"></i></button></div></div><div expr64="expr64" class="mt-2 flex items-center gap-2 text-sm cursor-pointer hover:bg-gray-700/30 -mx-2 px-2\n                            py-1 rounded transition-colors group/thread"></div></div>', [{
        expressions: [{
        ...
        getKey: null,
        condition: null,
        template: template('<span expr26="expr26"></span><div expr35="expr35" class="my-3 rounded-lg overflow-hidden border border-gray-700 bg-[#121016] shadow-inner"></div>', [{
          type: bindingTypes.IF,
...
@${_scope.segment.content}`
        ...
        getKey: null,
        condition: null,
        template: template('<div expr39="expr39" class="border border-gray-700 rounded-lg overflow-hidden bg-[#1A1D21] hover:border-gray-600 transition-colors max-w-lg"></div>', [{
          type: bindingTypes.IF,
          ...
          selector: '[expr39]',
          template: template('<a expr40="expr40" target="_blank" rel="noopener noreferrer" class="block"><div expr41="expr41" class="w-full h-48 bg-gray-800 border-b border-gray-700"></div><div class="p-3"><div class="flex items-center gap-2 mb-1"><img expr43="expr43" class="w-4 h-4 rounded"/><span expr44="expr44" class="text-xs text-gray-500"> </span></div><h4 expr45="expr45" class="text-sm font-semibold text-white line-clamp-1"> </h4><p expr46="expr46" class="text-xs text-gray-400 line-clamp-2 mt-1"></p></div></a>', [{
            ...
            selector: '[expr41]',
            template: template('<img expr42="expr42" class="w-full h-full object-cover"/>', [{
        ...
        selector: '[expr47]',
        template: template('<div class="bg-[#1A1D21] px-4 py-2 border-b border-gray-700 flex items-center justify-between"><span expr48="expr48" class="text-xs font-mono text-gray-500"> </span><span expr49="expr49" class="text-[10px] text-gray-600 uppercase tracking-widest font-bold"> </span></div><pre class="!p-0 !m-0 text-sm overflow-x-auto rounded-t-none"><code expr50="expr50"> </code></pre>', [{
        ...
        selector: '[expr51]',
        template: template('<div expr52="expr52" class="relative group/attachment"></div>', [{
          type: bindingTypes.EACH,
          getKey: null,
          condition: null,
          template: template('<template expr53="expr53"></template><template expr56="expr56"></template>', [{
            type: bindingTypes.IF,
            evaluate: _scope => _scope.isImage(_scope.attachment),
            redundantAttribute: 'expr53',
            selector: '[expr53]',
            template: template('<div expr54="expr54" class="block\n                                        cursor-pointer"><img expr55="expr55" class="max-w-xs max-h-64 rounded-lg border border-gray-700 hover:border-gray-500 transition-colors hover:opacity-90"/></div>', [{
            ...
            selector: '[expr56]',
            template: template('<a expr57="expr57" target="_blank" class="flex items-center p-2 rounded bg-[#222529] border border-gray-700 hover:border-gray-500 transition-colors text-blue-400 hover:text-blue-300"><svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/></svg><span expr58="expr58" class="text-sm truncate max-w-[150px]"> </span></a>', [{
        ...
        getKey: null,
        condition: null,
        template: template('<button expr60="expr60"> <span expr61="expr61" class="ml-1 text-gray-400"> </span></button>', [{
        ...
        selector: '[expr64]',
        template: template('<div class="flex -space-x-2"><div expr65="expr65"></div><div expr66="expr66" class="w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-bold\n                                    text-white bg-gray-600 border-2 border-[#1A1D21]"></div></div><span expr67="expr67" class="text-blue-400 group-hover/thread:underline font-medium"> </span><i class="fas fa-chevron-right text-xs text-gray-500 group-hover/thread:text-gray-400"></i>', [{
          expressions: [{
            type: expressionTypes.EVENT,
            name: 'onclick',
            evaluate: _scope => e => _scope.props.onOpenThread && _scope.props.onOpenThread(_scope.message, e)
          ...
          template: template(' ', [{
            expressions: [{
              type: expressionTypes.TEXT,
              childNodeIndex: 0,
              evaluate: _scope => [_scope.getInitials(_scope.participant)].join('')
            }, {
              type: expressionTypes.ATTRIBUTE,
              isBoolean: false,
              name: 'class',
    ...
    selector: '[expr68]',
    template: template('<button expr69="expr69" class="flex items-center space-x-2 bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-full shadow-lg transition-colors text-sm font-medium"><span>Read\n                    latest messages</span><i class="fas fa-arrow-down"></i></button>', [{
