...
var aiAgentModal = {
  ...
  template: (template, expressionTypes, bindingTypes, getComponent) => template('<div expr873="expr873" id="modalBackdrop" class="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm transition-all duration-300 ease-out opacity-0 hidden"><div class="absolute inset-0 bg-black/50 transition-opacity duration-300"></div><div expr874="expr874" id="modalContent" class="relative bg-gray-900/80 backdrop-blur-xl rounded-xl shadow-2xl w-full max-w-lg flex flex-col border border-white/10 overflow-hidden transform transition-all duration-300 ease-out scale-95 opacity-0 ring-1 ring-white/10"><div class="px-6 py-4 border-b border-gray-700/50 bg-gray-800/50 backdrop-blur-md sticky top-0 z-10"><h3 class="text-xl font-semibold text-white tracking-tight">Register New AI Agent</h3></div><div class="p-6 overflow-y-auto max-h-[80vh]"><div class="space-y-5"><div><label class="block text-sm font-medium text-gray-300 mb-2">Agent Name</label><input expr875="expr875" type="text" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:bg-gray-900 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors" placeholder="e.g. GPT-4 Worker"/><p expr876="expr876" class="mt-1 text-xs text-red-400 font-medium"></p></div><div><label class="block text-sm font-medium text-gray-300 mb-2">Agent Type</label><select expr877="expr877" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-100 focus:outline-none focus:bg-gray-900 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"><option value="generic">Generic</option><option value="text-generation">Text Generation</option><option value="image-generation">Image Generation</option><option value="data-analysis">Data Analysis</option><option value="custom">Custom</option></select></div><div expr878="expr878" class="animate-fade-in-down"></div><div><label class="block text-sm font-medium text-gray-300 mb-2">Initial Status</label><select expr880="expr880" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-100 focus:outline-none focus:bg-gray-900 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"><option value="idle">Idle</option><option value="busy">Busy</option><option value="offline">Offline</option></select></div><div><label class="block text-sm font-medium text-gray-300 mb-2">Webhook URL <span class="text-gray-500">(optional)</span></label><input expr881="expr881" type="url" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:bg-gray-900 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors" placeholder="https://api.example.com/agent/webhook"/><p class="mt-1 text-xs text-gray-500">Tasks will be POSTed to this URL when assigned to this agent.</p><p expr882="expr882" class="mt-1 text-xs text-red-400 font-medium"></p></div><div><label class="block text-sm font-medium text-gray-300 mb-2">Capabilities (JSON)</label><textarea expr883="expr883" rows="3" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-100 font-mono text-sm focus:outline-none focus:bg-gray-900 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"></textarea><p expr884="expr884" class="mt-1 text-xs text-red-400 font-medium"></p></div></div><div class="mt-8 flex justify-end space-x-3"><button expr885="expr885" type="button" class="px-4 py-2 text-sm font-medium text-gray-400 hover:text-white transition-colors hover:bg-gray-800/50 rounded-lg">\n                        Cancel\n                    </button><button expr886="expr886" type="button" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg shadow-lg shadow-indigo-600/20 transition-all disabled:opacity-50 disabled:shadow-none"> </button></div></div></div></div>', [{
    ...
    selector: '[expr878]',
    template: template('<input expr879="expr879" type="text" class="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:bg-gray-900 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors" placeholder="Custom type name"/>', [{
    ...
    expressions: [{
      type: expressionTypes.TEXT,
      childNodeIndex: 0,
      evaluate: _scope => [_scope.state.submitting ? 'Registering...' : 'Register Agent'].join('')
    }, {
      type: expressionTypes.EVENT,
      name: 'onclick',
...
export { aiAgentModal as default };
