export default {
  ...
  exports: {
    ...
    hide() {
      this.update({ visible: false, error: null })
      if (this.refs.nameInput) {
    ...
    onUpdated() {
      ...
      if (this.props.visible && this.refs.nameInput) {
        this.refs.nameInput.focus()
      }
    },
    ...
    async handleSubmit(e) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.update({ error: error.error || 'Failed to create database' })
        }
      } catch (error) {
  ...
  template: (
    ...
    '<div expr192="expr192" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"></div>',
    [
      {
          ...
          [
            {
              ...
              template: template(
                '<div class="flex items-start"><svg class="h-5 w-5 text-red-400 mr-2 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><p expr194="expr194" class="text-sm text-red-300"> </p></div>',
                [
                  {
